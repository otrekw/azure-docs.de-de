...
**Detail**: Verwenden Sie vordefinierte RBAC-Rollen. Wenn Sie beispielsweise einem Benutzer Zugriff für die Verwaltung von Key Vault-Instanzen gewähren möchten, weisen Sie ihm für einen bestimmten Bereich die vordefinierte Rolle [Key Vault-Mitwirkender](/azure/role-based-access-control/built-in-roles) zu. Der Bereich wäre in diesem Fall ein Abonnement, eine Ressourcengruppe oder einfach eine bestimmte Key Vault-Instanz. Wenn die vordefinierten Rollen nicht Ihren Anforderungen entsprechen, können Sie [eigene Rollen definieren](/azure/role-based-access-control/custom-roles).
...
**Detail**: Verwenden Sie [Azure Disk Encryption](/azure/security/azure-security-disk-encryption-overview). Damit können IT-Administratoren die Datenträger virtueller IaaS-Computer unter Windows und Linux verschlüsseln. Disk Encryption kombiniert die Branchenstandardfeatures BitLocker (Windows) und dm-crypt (Linux), um Volumeverschlüsselung für das Betriebssystem und die Datenträger bereitzustellen.
...
**Detail**: Verwenden Sie [Site-to-Site-VPN](/azure/vpn-gateway/vpn-gateway-howto-site-to-site-resource-manager-portal).
