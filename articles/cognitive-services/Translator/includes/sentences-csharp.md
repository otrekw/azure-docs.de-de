...
Erstellen Sie in der Klasse `Program` eine neue Funktion mit dem Namen `BreakSentenceRequest()`. Diese Funktion verwendet vier Argumente: `subscriptionKey`, `endpoint`, `route` und `inputText`.
...
Sie erkennen, dass Sie im `Main`-Element `subscriptionKey`, `endpoint`, `route` und den Text zum Auswerten von `breakSentenceText` deklarieren.
