...
Als Nächstes wird eine Gruppe von Klassen erstellt, die bei der Deserialisierung der von Translator zurückgegebenen JSON-Antwort verwendet werden.
...
Erstellen Sie in der Klasse `Program` eine asynchrone Funktion namens `TranslateTextRequest()`. Diese Funktion verwendet vier Argumente: `subscriptionKey`, `host`, `route` und `inputText`.
