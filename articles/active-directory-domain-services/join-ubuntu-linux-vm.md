...
Während der Kerberos-Installation werden Sie vom Paket *krb5-user* aufgefordert, den Bereichsnamen in Großbuchstaben einzugeben. Wenn der Name Ihrer durch Azure AD DS verwalteten Domäne z. B. *aaddscontoso.com* lautet, geben Sie als Bereich *AADDSCONTOSO.COM* ein. Die Installation schreibt die Abschnitte `[realm]` und `[domain_realm]` in die Konfigurationsdatei */etc/krb5.conf*. Stellen Sie sicher, dass Sie den Bereich in Großbuchstaben angeben:
...
1. Erstellen Sie in der Datei *ntp.conf* eine Zeile, um den DNS-Namen Ihrer durch Azure AD DS verwalteten Domäne hinzuzufügen. Im folgenden Beispiel wird ein Eintrag für *aaddscontoso.com* hinzugefügt. Verwenden Sie den eigenen DNS-Namen:
