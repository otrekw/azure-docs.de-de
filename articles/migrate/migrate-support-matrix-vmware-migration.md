...
**vCenter Server-Berechtigungen** | Bei der Migration ohne Agent wird die [Migrate-Appliance](migrate-appliance.md) verwendet. Für die Appliance sind folgende Berechtigungen in vCenter Server erforderlich:<br/><br/> - **Datastore.Browse** (Datenspeicher -> Datenspeicher durchsuchen): Ermöglicht das Durchsuchen von VM-Protokolldateien zur Fehlerbehebung bei der Erstellung und Löschung von Momentaufnahmen.<br/><br/> - **Datastore.FileManagement** (Datenspeicher -> Low-Level-Dateivorgänge): Ermöglicht Lese-/Schreib-/Lösch-/Umbenennungsvorgänge im Datenspeicherbrowser, um Probleme bei der Erstellung und Löschung von Momentaufnahmen zu beheben.<br/><br/> - **VirtualMachine.Config.ChangeTracking** (Virtueller Computer -> Änderungsnachverfolgung für Datenträger): Ermöglicht die Aktivierung oder Deaktivierung der Änderungsverfolgung von VM-Datenträgern, um geänderte Datenblöcke zwischen Momentaufnahmen abzurufen.<br/><br/> - **VirtualMachine.Config.DiskLease** (Virtueller Computer -> Datenträgerlease): Ermöglicht Datenträger-Leasevorgänge für eine VM, damit der Datenträger mit dem VMware vSphere Virtual Disk Development Kit (VDDK) gelesen werden kann.<br/><br/> - **VirtualMachine.Provisioning.DiskRandomRead** (Virtueller Computer -> Bereitstellung -> Schreibgeschützten Datenträgerzugriff zulassen): Ermöglicht das Öffnen eines Datenträgers auf einer VM, damit der Datenträger mit dem VDDK gelesen werden kann.<br/><br/> - **VirtualMachine.Provisioning.DiskRandomAccess** (Virtueller Computer -> Bereitstellung -> Datenträgerzugriff zulassen): Ermöglicht das Öffnen eines Datenträgers auf einer VM, damit der Datenträger mit dem VDDK gelesen werden kann.<br/><br/> - **VirtualMachine.Provisioning.GetVmFiles** (Virtueller Computer -> Bereitstellung -> Download virtueller Computer zulassen): Ermöglicht Lesevorgänge für Dateien, die einer VM zugeordnet sind, um die Protokolle herunterzuladen und ggf. Fehler zu beheben.<br/><br/> - **VirtualMachine.State.\* *_ (Virtueller Computer -> Momentaufnahmenverwaltung): Ermöglicht das Erstellen und Verwalten von VM-Momentaufnahmen zur Replikation.<br/><br/> - _* VirtualMachine.Interact.PowerOff** (Virtueller Computer -> Interaktion -> Ausschalten): Ermöglicht das Ausschalten der VM während der Migration zu Azure.
