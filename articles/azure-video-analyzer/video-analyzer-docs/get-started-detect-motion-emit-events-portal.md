...
ms.locfileid: "112079533"
---
# <a name="quickstart-get-started-with-azure-video-analyzer"></a>Schnellstart: Erste Schritte mit Azure Video Analyzer
In dieser Schnellstartanleitung werden die ersten Schritte mit Azure Video Analyzer erläutert. Erstellen Sie im Azure-Portal ein Azure Video Analyzer-Konto und die zugehörigen Ressourcen.
Nachdem Sie Ihr Video Analyzer-Konto erstellt haben, stellen Sie das Video Analyzer-Edgemodul und ein RTSP-Kamerasimulatormodul auf Ihrem IoT Edge-Gerät bereit.
...
## <a name="preparing-your-iot-edge-device"></a>Vorbereitung des IoT Edge-Geräts
Das Azure Video Analyzer-Modul sollte für die Ausführung auf dem IoT Edge-Gerät mit einem nicht privilegierten lokalen Benutzerkonto konfiguriert werden. Für das Modul werden bestimmte lokale Ordner benötigt, um die Daten für die Anwendungskonfiguration zu speichern. Für das RTSP-Kamerasimulatormodul werden Videodateien benötigt, mit denen ein Livevideofeed synthetisiert werden kann.
...
## <a name="creating-azure-resources"></a>Erstellen von Azure-Ressourcen
Im nächsten Schritt erstellen Sie die erforderlichen Azure-Ressourcen (Video Analyzer-Konto, Speicherkonto, benutzerseitig zugewiesene verwaltete Identität). Anschließend erstellen Sie eine optionale Containerregistrierung und registrieren ein Video Analyzer-Edgemodul beim Video Analyzer-Konto.
...
1. Geben Sie im Abschnitt zum **Erstellen eines Video Analyzer-Kontos** die erforderlichen Werte ein.
    - **Abonnement:** Wählen Sie das Abonnement aus, unter dem das Video Analyzer-Konto erstellt werden soll.
    - **Ressourcengruppe:** Wählen Sie eine Ressourcengruppe aus, in der das Video Analyzer-Konto erstellt werden soll, oder klicken Sie auf **Neu erstellen**, um eine neue Ressourcengruppe zu erstellen.
    - **Name des Video Analyzer-Kontos:** Dies ist der Name für Ihr Video Analyzer-Konto. Der Name darf nur aus drei bis 24 Kleinbuchstaben oder Zahlen bestehen und darf keine Leerzeichen enthalten.
    - **Speicherort:** Wählen Sie einen Speicherort zum Bereitstellen Ihres Video Analyzer-Kontos aus, z. B. **USA, Westen 2**.
    - **Speicherkonto:** Erstellen Sie ein neues Speicherkonto. Es wird empfohlen, ein [allgemeines Standardspeicherkonto der Version 2](../../storage/common/storage-account-overview.md#types-of-storage-accounts) auszuwählen.
...
1. Geben Sie auf der Registerkarte **Basics** (Grundeinstellungen) Werte für **Ressourcengruppe** **_(verwenden Sie die _*Ressourcengruppe** aus den vorherigen Abschnitten) **_ und _*Registrierungsname** ein. Der Registrierungsname muss innerhalb von Azure eindeutig sein und zwischen 5 und 50 alphanumerische Zeichen enthalten.
...
## <a name="deploying-edge-modules"></a>Bereitstellen von Edgemodulen

### <a name="deploying-video-analyzer-edge-module"></a>Bereitstellen des Video Analyzer-Edgemoduls

1. Navigieren Sie zu Ihrem Video Analyzer-Konto.
1. Wählen Sie auf dem Blatt **Edge** die Option **Edgemodule** aus.
1. Wählen Sie **Edgemodule hinzufügen** aus. Geben Sie **_avaedge_ *_ als Name für das neue Edgemodul ein, und wählen Sie _* Hinzufügen** aus.
1. Die Anzeige **Kopieren Sie das Bereitstellungstoken** wird auf der rechten Seite des Bildschirms eingeblendet.
1. Kopieren Sie den Codeausschnitt unter **Empfohlene gewünschte Eigenschaften für die IoT-Modulbereitstellung**. Diesen benötigen Sie in einem späteren Schritt.
    ```JSON
      {
...
1. Wählen Sie unter der **automatische Geräteverwaltung** die Option **IoT Edge** aus.
1. Wählen Sie die **Geräte-ID** für Ihr IoT Edge-Gerät aus.
1. Wählen Sie **Module festlegen** aus.
1. Wählen Sie **Hinzufügen** und dann im Dropdownmenü **IoT Edge-Modul** aus.
1. Geben Sie **avaedge** als **Name des IoT Edge-Moduls** ein.
...
1. Geben Sie unter **NAME** den Namen **LOCAL_USER_ID** und unter **VALUE** (WERT) den Wert **1010** ein.
1. Geben Sie in der zweiten Zeile unter **NAME** den Namen **LOCAL_GROUP_ID** und unter **VALUE** (WERT) den Wert **1010** ein.
1. Wählen Sie **Optionen für Containererstellung** aus. Kopieren Sie die folgenden Zeilen, und fügen Sie sie ein:  
    ...
    ```
1. Wählen Sie **Einstellungen für Modulzwilling** aus, und fügen Sie den Codeausschnitt ein, den Sie zuvor unter dem Video Analyzer-Konto auf der Seite **Kopieren Sie das Bereitstellungstoken** kopiert haben.
    ```JSON
      {
          "applicationDataDirectory": "/var/lib/videoanalyzer",
          "ProvisioningToken": "XXXXXXX",
    ...
    ```
1. Wählen Sie am unteren Bildschirmrand **Hinzufügen** aus.
1. Wählen Sie **Routen** aus.
1. Geben Sie unter **NAME** den Namen **AVAToHub** und unter **VALUE** (WERT) „FROM /messages/modules/avaedge/outputs/* INTO $upstream“ ein.
1. Wählen Sie zunächst **Überprüfen und erstellen** und dann **Erstellen** aus. Daraufhin wird das Edgemodul **avaedge** bereitgestellt.

### <a name="deploying-rtsp-camera-simulator-edge-module"></a>Bereitstellen des Edgemoduls des RTSP-Kamerasimulators
1. Navigieren Sie zu Ihrer IoT Hub-Instanz.
1. Wählen Sie unter der **automatische Geräteverwaltung** die Option **IoT Edge** aus.
1. Wählen Sie die **Geräte-ID** für Ihr IoT Edge-Gerät aus.
1. Wählen Sie **Module festlegen** aus.
1. Wählen Sie **Hinzufügen** und dann im Dropdownmenü **IoT Edge-Modul** aus.
1. Geben Sie **rtspsim** als **Name des IoT Edge-Moduls** ein.
1. Kopieren Sie die folgende Zeile, und fügen Sie sie in das Feld **Image-URI** ein: `mcr.microsoft.com/lva-utilities/rtspsim-live555:1.2`
1. Wählen Sie **Optionen für Containererstellung** aus. Kopieren Sie die folgenden Zeilen, und fügen Sie sie ein:  
    ```json
            {
                "HostConfig": {
    ...
    ```
1. Wählen Sie am unteren Bildschirmrand **Hinzufügen** aus.
1. Wählen Sie zunächst **Überprüfen und erstellen** und dann **Erstellen** aus. Daraufhin wird das Edgemodul **rtspsim** bereitgestellt.
...
1. Klicken Sie mit der rechten Maustaste auf die **Geräte-ID**, und wählen Sie **Überwachung des integrierten Ereignisendpunkts starten** aus.
