...
* Es wurde ein Azure Storage-Konto mit dem Resource Manager-Bereitstellungsmodell in dem Abonnement erstellt, aus dem Sie die Datei mit den Veröffentlichungseinstellungen heruntergeladen haben.
...
* Das Tool *AzureOfflineBackup.exe* generiert eine *OfflineApplicationParams.xml*-Datei. Kopieren Sie diese Datei mit dem MABS oder DPM auf den Server.
...
1. Wählen Sie die Registerkarte **Computerzertifikatsanwendung verwalten** > **Persönlich** aus. Suchen Sie nach dem Zertifikat mit dem Namen `CB_AzureADCertforOfflineSeeding_<ResourceId>`.
...
1. Schließen Sie den Workflow ab. Wählen Sie in der Verwaltungskonsole des Azure Backup-Agents **Jetzt sichern** aus, um die Offlinesicherungskopie zu initiieren. Die erste Sicherung wird bei diesem Schritt in den Stagingbereich geschrieben.
    ...
    ![DPM und MABS – Jetzt sichern](./media/backup-azure-backup-import-export/dpmbackupnow.png)
...
1. Die Ausgabe zeigt den aktuellen Status des Importauftrags an.
    ...
    ![Importauftragsstatus überprüfen](./media/backup-azure-backup-import-export/importjobstatusreporting.png)<br/>
