...
| Vorgang | Fehlerdetails | Problemumgehung |
| --- | --- | --- |
| Übertragen von Agents auf geschützte Server mithilfe von Push | Fehler beim Agent-Vorgang. Bei der Kommunikation mit dem DPM-Agent-Koordinator-Dienst auf \<ServerName> ist ein Fehler aufgetreten. | **Falls die im Produkt empfohlene Aktion nicht funktioniert, können Sie die folgenden Schritte ausführen**: <ul><li> Falls Sie einen Computer aus einer nicht vertrauenswürdigen Domäne anfügen, führen Sie [diese Schritte](/system-center/dpm/back-up-machines-in-workgroups-and-untrusted-domains?view=sc-dpm-2019) aus. <br> (ODER) </li><li> Falls Sie einen Computer aus einer vertrauenswürdigen Domäne anfügen, verwenden Sie die Problembehandlungsschritte aus [diesem Blog](https://techcommunity.microsoft.com/t5/system-center-blog/data-protection-manager-agent-network-troubleshooting/ba-p/344726). <br>(ODER)</li><li> Deaktivieren Sie die Antivirensoftware als Problembehandlungsschritt. Wird das Problem dadurch behoben, ändern Sie die Antivireneinstellungen, wie in [diesem Artikel](/system-center/dpm/run-antivirus-server?view=sc-dpm-2019) beschrieben.</li></ul> |
...
| Backup | Fehler beim Zugreifen auf Dateien/freigegebene Ordner | Versuchen Sie, die Einstellungen der Antivirensoftware zu ändern (wie in diesem Artikel [Ausführen von Antivirensoftware auf dem DPM-Server](/system-center/dpm/run-antivirus-server?view=sc-dpm-2019) beschrieben).|
