...
| Fehlermeldung | Mögliche Ursachen | Empfohlene Maßnahme |
|---|---|---|
| Der Vorgang wurde abgebrochen, da in derselben Datenbank bereits ein widersprüchlicher Vorgang ausgeführt wurde. | In den folgenden Fällen kann dieser Fehlercode angezeigt werden:<br><ul><li>Hinzufügen oder Ablegen von Dateien zu bzw. in einer Datenbank während einer Sicherung</li><li>Verkleinern von Dateien während Datenbanksicherungen</li><li>Auslösung eines Sicherungsauftrags durch eine Azure Backup-Erweiterung während einer Datenbanksicherung durch ein anderes für die Datenbank konfiguriertes Sicherungsprodukt</li></ul>| Deaktivieren Sie das andere Sicherungsprodukt, um das Problem zu beheben.
...
|---|---|---|
| Sicherungen, Dateibearbeitungsvorgänge (z. B. ALTER DATABASE ADD FILE) und Verschlüsselungsänderungen in einer Datenbank müssen serialisiert werden. | Dieser Fehler tritt möglicherweise auf, wenn der ausgelöste bedarfsgesteuerte oder geplante Sicherungsauftrag im Konflikt mit einem bereits laufenden Sicherungsvorgang steht, der von einer Azure Backup-Erweiterung für dieselbe Datenbank ausgelöst wurde.<br> Im Folgenden finden Sie die Szenarien, in denen dieser Fehlercode angezeigt werden kann:<br><ul><li>Für die Datenbank wird eine vollständige Sicherung ausgeführt, und es wird eine weitere vollständige Sicherung ausgelöst.</li><li>Für die Datenbank wird eine Diff-Sicherung ausgeführt, und es wird eine weitere Diff-Sicherung ausgelöst.</li><li>Für die Datenbank wird eine Protokollsicherung ausgeführt, und es wird eine weitere Protokollsicherung ausgelöst.</li></ul>| Nachdem bei dem widersprüchlichen Vorgang ein Fehler aufgetreten ist, wiederholen Sie den Vorgang.
