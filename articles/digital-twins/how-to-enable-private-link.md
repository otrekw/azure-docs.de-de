...
Sie können Private Link mit einem privaten Endpunkt für eine Azure Digital Twins-Instanz als Teil der ersten Einrichtung der Instanz aktivieren. Alternativ können Sie Private Link auch später für eine bereits vorhandene Instanz aktivieren. 

Beide dieser Erstellungsmethoden bieten Ihnen dieselben Konfigurationsoptionen und dasselbe Endergebnis für Ihre Instanz. In diesem Abschnitt wird die Durchführung beider Methoden im [Azure-Portal](https://portal.azure.com) beschrieben.
...
In diesem Abschnitt verwenden Sie das [Azure-Portal](https://portal.azure.com), um Private Link mit einem privaten Endpunkt für eine bereits vorhandene Azure Digital Twins-Instanz zu aktivieren.
...
Sie können den Wert des Netzwerkflags mithilfe des [Azure-Portals](https://portal.azure.com), der [Azure CLI](/cli/azure/) oder des [ARMClient-Befehlstools](https://github.com/projectkudu/ARMClient) ändern.

### <a name="option-1-using-the-azure-portal"></a>Option 1: Verwenden des Azure-Portals
...
### <a name="option-2-using-the-azure-cli"></a>Option 2: Verwenden der Azure-Befehlszeilenschnittstelle

Wie unten gezeigt erfolgt diese Aktion über die Azure CLI mithilfe des Befehls `az resource update`. Die Eigenschaft „publicNetworkAccess“ einer Azure Digital Twins-Instanz kann auf `disabled` oder `enabled` festgelegt werden.

Verwenden Sie den folgenden Befehl zum Deaktivieren des Flags, indem Sie die Platzhalter durch Ihre Instanzwerte ersetzen.
...
Im folgenden Screenshot wird gezeigt, wie die Ausgabe aussieht.

:::image type="content" source="media/how-to-enable-private-link/network-flag-cli.png" alt-text="Screenshot: Terminalfenster mit Ausführung des oben gezeigten Befehls, die Ausgabe enthält Details der Instanz, einschließlich der Eigenschaft „publicNetworkAccess“ mit dem Wert „Disabled“" lightbox="media/how-to-enable-private-link/network-flag-cli.png":::

Verwenden Sie den folgenden ähnlichen Befehl, um das Flag zu aktivieren.
...
### <a name="option-3-usingarmclient"></a>Option 3: Verwenden von ARMClient  
