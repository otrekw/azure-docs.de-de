...
description: Erfahren Sie mehr über Sicherheitsempfehlungen für Blob Storage. Durch die Umsetzung dieser Anleitung können Sie die Sicherheitsverpflichtungen erfüllen, die in unserem Modell der gemeinsamen Verantwortung beschrieben sind.
services: storage
author: tamram
ms.service: storage
ms.subservice: blobs
...
| Schützen Ihrer Kontozugriffsschlüssel mit Azure Key Vault | Microsoft empfiehlt die Verwendung von Azure AD zum Autorisieren von Anforderungen an Azure Storage. Wenn jedoch eine Autorisierung mit gemeinsam verwendetem Schlüssel erforderlich ist, schützen Sie Ihre Kontoschlüssel mit Azure Key Vault. Sie können die Schlüssel zur Laufzeit aus dem Schlüsseltresor abrufen, anstatt Sie mit der Anwendung zu speichern. Weitere Informationen zu Azure Key Vault finden Sie in der [Übersicht über Azure Key Vault](../../key-vault/key-vault-overview.md). | - |
