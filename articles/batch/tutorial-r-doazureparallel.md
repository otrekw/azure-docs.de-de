---
title: Tutorial – Ausführen einer parallelen R-Simulation mit Azure Batch
...
* Erhöhen Sie `taskSlotsPerNode` auf *2*, um auf jedem Knoten beide Kerne zu nutzen.
...
In der Ausgabe wird die Anzahl von „Ausführungsworkern“ für doAzureParallel angezeigt. Dies ist die Anzahl von Knoten multipliziert mit dem Wert von `taskSlotsPerNode`. Wenn Sie die Clusterkonfiguration wie oben beschrieben geändert haben, beträgt die Anzahl *10*.
...
![Screenshot: Histogramm der Schlusskurse](media/tutorial-r-doazureparallel/closing-prices-local.png)
