...
Titel: Streamen von Inhalten mit CDN-Integration: Azure Media Services description: Erfahren Sie mehr über das Streamen von Inhalten mit CDN-Integration sowie den Vorabruf und den CDN-Vorabruf mit Ursprungsunterstützung.
...
CDN speichert Inhalte zwischen, die von einem Media Services-[Streamingendpunkt (Ursprung)](stream-streaming-endpoint-concept.md) per Codec, per Streamingprotokoll, per Bitrate, per Containerformat und per Verschlüsselung/DRM gestreamt werden. Für jede Kombination von Codec-Streamingprotokoll-Containerformat-Bitrate-Verschlüsselung wird es einen separaten CDN-Cache geben.
...
- Der [Streamingendpunkt](stream-streaming-endpoint-concept.md) `hostname` und die Streaming-URL ändern sich nicht, und zwar unabhängig davon, ob Sie CDN aktivieren oder nicht.
...
* Lesen Sie unbedingt das Dokument [Streamingendpunkt (Ursprung)](stream-streaming-endpoint-concept.md).
