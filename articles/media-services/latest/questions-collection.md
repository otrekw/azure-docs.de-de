---
title: 'Azure Media Services v3: Fragensammlung'
description: Dieser Artikel enthält Antworten auf verschiedene Fragen zu Azure Media Services v3.
...
# <a name="media-services-v3-questions-collection"></a>Azure Media Services v3: Fragensammlung
...
Bei Verwendung der Paginierung sollten Sie immer den Link „Weiter“ verwenden, um die Sammlung zu durchlaufen, und nicht auf eine bestimmte Seitengröße zurückgreifen. Weitere Informationen und Beispiele finden Sie unter [Filterung, Sortierung, Paginierung](filter-order-page-entitites-how-to.md).
...
Sie können einen [lokalen Liveencoder](encode-recommended-on-premises-live-encoders.md) verwenden, um das Quellvideo zu wechseln. Viele Apps bieten die Möglichkeit zum Wechseln der Quelle, darunter Telestream Wirecast, Switcher Studio (unter iOS) und OBS Studio (kostenlose App).
...
* Verwalten von [Liveereignissen](live-event-outputs-concept.md) in Media Services v3 
...
Verwenden Sie für alle anderen Verwaltungsaufgaben (z. B. [Transformationen und Aufträge](transform-jobs-concept.md) und [Inhaltsschutz](drm-content-protection-concept.md)) die [REST-API](/rest/api/media/), die [Azure-Befehlszeilenschnittstelle](/cli/azure/ams) oder eines der unterstützten [SDKs](media-services-apis-overview.md#sdks).
