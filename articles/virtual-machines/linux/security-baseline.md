...
ms.locfileid: "107285338"
---
# <a name="azure-security-baseline-for-linux-virtual-machines"></a>Azure-Sicherheitsbaseline für Virtuelle Linux-Computer

Diese Sicherheitsbaseline wendet Anleitungsempfehlungen aus [Version 1.0 von Azure Security Benchmark](../../security/benchmarks/overview-v1.md) auf virtuelle Linux-Computer an. Der Azure-Sicherheitsvergleichstest enthält Empfehlungen zum Schutz Ihrer Cloudlösungen in Azure. Der Inhalt ist nach den **Sicherheitskontrollen** gruppiert, die durch den Azure-Sicherheitsvergleichstest und die entsprechenden, für virtuelle Linux-Computer geltenden Empfehlungen definiert werden. 

> [!NOTE]
> Nicht auf virtuelle Linux-Computer anwendbare **Steuerungen** oder Steuerungen, für die Microsoft die Verantwortung trägt, wurden ausgeschlossen. Die gesamte Zuordnung zwischen virtuellen Linux-Computern und dem Azure-Sicherheitsvergleichstest finden Sie in der **[vollständigen Zuordnungsdatei der Sicherheitsbaseline für virtuelle Linux-Computer](https://github.com/MicrosoftDocs/SecurityBenchmarks/raw/master/Azure%20Offer%20Security%20Baselines/1.1/linux-virtual-machines-security-baseline-v1.1.xlsx)** .
...
- [Azure Policy-Beispiele für Netzwerke](/azure/governance/policy/samples/built-in-policies#network)
...
- [Um die Syslog-Daten für die Überwachung zu erfassen, müssen Sie die Log Analytics-Erweiterung aktivieren.](https://docs.microsoft.com/azure/azure-monitor/vm/quick-collect-azurevm#enable-the-log-analytics-vm-extension)
...
Für erweiterte Einblicke in die von Ihren virtuellen Computern unterstützten Anwendungen und Dienste können Sie Azure Monitor für VMs und Application Insights aktivieren. Mit Application Insights überwachen Sie Ihre Anwendung und erfassen Telemetriedaten wie HTTP-Anforderungen, Ausnahmen usw., damit Sie Probleme zwischen den VMs und Ihrer Anwendung korrelieren können.
...
- [Übersicht über den Log Analytics-Agent](/azure/azure-monitor/platform/log-analytics-agent)
...
- [Der folgende Link enthält die empfohlenen Sicherheitsrichtlinien von Microsoft, die als Kriterienliste für die ausgewählte Sicherheitsrisikosoftware dienen können.](/azure/virtual-machines/linux/security-recommendations)
...
**Leitfaden**: Auf virtuellen Linux-Computern und in Azure Active Directory (Azure AD) gibt es konzeptionell keine Standardkennwörter. Der Kunde ist verantwortlich für Anwendungen und Marketplace-Dienste von Drittanbietern, bei denen möglicherweise Standardkennwörter verwendet werden.
...
**Leitfaden**: Verwenden Sie die Richtlinien und benannten Standorte der Azure Active Directory-Funktion (Azure AD) „Bedingter Zugriff“, um den Zugriff nur bestimmten logischen Gruppierungen von IP-Adressbereichen oder Ländern/Regionen zu gestatten.
...
**Leitfaden**: Konfigurieren Sie Diagnoseeinstellungen für Azure Active Directory (Azure AD), um die Überwachungs- und Anmeldeprotokolle an einen Log Analytics-Arbeitsbereich zu senden. Verwenden Sie außerdem Azure Monitor, um Protokolle zu überprüfen und Abfragen für Syslog-Daten von virtuellen Azure-Computern auszuführen.
...
**Leitfaden**: Mit Azure AD-Features (Azure Active Directory) zum Risiko- und Identitätsschutz können Sie automatische Reaktionen auf erkannte verdächtige Aktionen im Zusammenhang mit Speicherkontoressourcen konfigurieren. Sie sollten automatisierte Antworten über Azure Sentinel aktivieren, um die Sicherheitsmaßnahmen Ihrer Organisation zu implementieren.
...
- [Von Microsoft empfohlene Sicherheitsrichtlinien](/azure/virtual-machines/linux/security-recommendations)
...
- [Updateverwaltung in Azure](/azure/automation/overview)

- [Verwalten von Updates und Patches für Ihre Azure-VMs](/azure/automation/manage-updates-for-vm)
...
- [Hochladen einer benutzerdefinierten VM-VHD in Azure](../windows/upload-generalized-managed.md)
...
- [Konfigurieren von Microsoft Antimalware für Cloud Services und Virtual Machines](/azure/virtual-machines/linux/security-recommendations)
