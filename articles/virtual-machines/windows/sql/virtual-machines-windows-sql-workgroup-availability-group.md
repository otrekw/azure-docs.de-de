...
1. Ändern Sie den Namen der Arbeitsgruppe in einen aussagekräftigen Namen, z. B. `AGWORKGROUP`: 
   ...
   ![Ändern des Arbeitsgruppennamens](media/virtual-machines-windows-sql-workgroup-availability-group/1-change-workgroup-name.png)
...
1. Geben Sie unter **Primäres DNS-Suffix des Computers** den Namen des DNS-Suffixes ein, z. B. `ag.wgcluster.example.com`, und wählen Sie dann **OK** aus: 
   ...
   ![Hinzufügen des DNS-Suffix](media/virtual-machines-windows-sql-workgroup-availability-group/2-add-dns-suffix.png)
...
1. Vergewissern Sie sich, dass unter **Vollständiger Computername** nun das DNS-Suffix angezeigt wird, und wählen Sie dann **OK** aus, um die Änderungen zu speichern: 
   ...
   ![Hinzufügen des DNS-Suffix](media/virtual-machines-windows-sql-workgroup-availability-group/3-confirm-full-computer-name.png)
...
1. Fügen Sie am Ende der Datei einen Eintrag für jeden Knoten, die Verfügbarkeitsgruppe und den Listener in der Form `IP Address, DNS Suffix #comment` wie folgt hinzu: 
   ...
   ![Hinzufügen von Einträgen für IP-Adresse, Cluster und Listener in der Hostdatei](media/virtual-machines-windows-sql-workgroup-availability-group/4-host-file.png)
...
In diesem Schritt erstellen Sie den Failovercluster. Wenn Sie mit diesen Schritten nicht vertraut sind, können Sie sich im [Tutorial für Failovercluster](virtual-machines-windows-portal-sql-create-failover-cluster.md#step-2-configure-the-windows-server-failover-cluster-with-storage-spaces-direct) darüber informieren.
...
1. Öffnen Sie auf einem der Knoten den **Failovercluster-Manager**, wählen Sie den Cluster aus, klicken Sie mit der rechten Maustaste auf **Name: \<Clustername>** unter **Hauptressourcen des Clusters**, und wählen Sie dann **Eigenschaften** aus. 

   ![Öffnen von „Eigenschaften“ für den Clusternamen](media/virtual-machines-windows-sql-workgroup-availability-group/5-launch-cluster-name-properties.png)
...
1. Wählen Sie **Statische verwenden** aus, geben Sie die IP-Adresse des Clusters an, und wählen Sie dann **OK** aus: 
   ...
   ![Angeben einer statischen IP-Adresse für den Cluster](media/virtual-machines-windows-sql-workgroup-availability-group/6-provide-static-ip-for-cluster.png)

1. Vergewissern Sie sich, dass Ihre Einstellungen korrekt aussehen, und wählen Sie dann **OK** aus, um sie zu speichern:

   ![Überprüfen der Clustereigenschaften](media/virtual-machines-windows-sql-workgroup-availability-group/7-verify-cluster-properties.png)

## <a name="create-a-cloud-witness"></a>Erstellen eines Cloudzeugen 

In diesem Schritt konfigurieren Sie einen Cloudfreigabezeugen. Wenn Sie mit den Schritten nicht vertraut sind, finden Sie weitere Informationen im [Tutorial zu Failoverclustern](virtual-machines-windows-portal-sql-create-failover-cluster.md#create-a-cloud-witness). 

## <a name="enable-availability-group-feature"></a>Aktivieren des Features für Verfügbarkeitsgruppen 

In diesem Schritt aktivieren Sie das Feature für Verfügbarkeitsgruppen. Wenn Sie mit den Schritten nicht vertraut sind, finden Sie weitere Informationen im [Tutorial zu Verfügbarkeitsgruppen](virtual-machines-windows-portal-sql-availability-group-tutorial.md#enable-availability-groups). 
...
In diesem letzten Schritt konfigurieren Sie den Lastenausgleich im [Azure-Portal](virtual-machines-windows-portal-sql-alwayson-int-listener.md) oder mit [PowerShell](virtual-machines-windows-portal-sql-ps-alwayson-int-listener.md).


## <a name="next-steps"></a>Nächste Schritte

Sie können auch die [Azure-Befehlszeilenschnittstelle auf der SQL-VM](virtual-machines-windows-sql-availability-group-cli.md) verwenden, um eine Verfügbarkeitsgruppe zu konfigurieren. 
