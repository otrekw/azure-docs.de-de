---
title: Azure-Sicherheitsbaseline für virtuelle Windows-Computer
...
ms.locfileid: "107284924"
---
# <a name="azure-security-baseline-for-windows-virtual-machines"></a>Azure-Sicherheitsbaseline für virtuelle Windows-Computer

Diese Sicherheitsbaseline wendet Empfehlungen aus [Version 1.0 des Vergleichstests für die Azure-Sicherheit](../../security/benchmarks/overview-v1.md) auf virtuelle Windows-Computer an. Der Azure-Sicherheitsvergleichstest enthält Empfehlungen zum Schutz Ihrer Cloudlösungen in Azure. Der Inhalt ist nach den **Sicherheitskontrollen** gruppiert, die durch den Vergleichstest für die Azure-Sicherheit und die entsprechenden, für virtuelle Windows-Computer geltenden Empfehlungen definiert werden.

> [!NOTE]
> Nicht auf virtuelle Windows-Computer anwendbare **Kontrollen** oder Kontrollen, für die Microsoft die Verantwortung trägt, wurden ausgeschlossen. Die gesamte Zuordnung zwischen virtuellen Windows-Computern und dem Vergleichstest für die Azure-Sicherheit finden Sie in der **[vollständigen Zuordnungsdatei der Sicherheitsbaseline für virtuelle Windows-Computer](https://github.com/MicrosoftDocs/SecurityBenchmarks/blob/master/Azure%20Offer%20Security%20Baselines/1.1/windows-virtual-machines-security-baseline-v1.1.xlsx)** .
...
- [Sammeln von Plattformprotokollen und -metriken mit Azure Monitor](https://docs.microsoft.com/azure/azure-monitor/essentials/activity-log#view-the-activity-log)
...
**Leitfaden**: Auf virtuellen Windows-Computern und in Azure Active Directory (Azure AD) gibt es konzeptionell keine Standardkennwörter. Der Kunde ist verantwortlich für Anwendungen und Marketplace-Dienste von Drittanbietern, bei denen möglicherweise Standardkennwörter verwendet werden.
...
- [Serverseitige Verschlüsselung von verwalteten Azure-Datenträgern](/azure/virtual-machines/windows/disk-encryption)

- [Azure Disk Encryption für Windows-VMs](/azure/virtual-machines/windows/disk-encryption-overview)
...
Zusätzlich zur Änderungsnachverfolgung für die Überwachung von Softwareanwendungen verwenden adaptive Anwendungssteuerungen in Azure Security Center maschinelles Lernen, um die auf Ihren Computern ausgeführten Anwendungen zu analysieren und anhand dieser Informationen eine Zulassungsliste zu erstellen. Dadurch wird das Konfigurieren und Verwalten von Richtlinien für die Anwendungszulassungsliste erheblich vereinfacht, sodass Sie die Verwendung unerwünschter Software in Ihrer Umgebung verhindern können. Sie können den Überwachungsmodus oder den Erzwingungsmodus konfigurieren. Im Überwachungsmodus werden nur die Aktivitäten auf den geschützten VMs überwacht. Im Erzwingungsmodus werden die Regeln erzwungen, und Anwendungen, deren Ausführung nicht zulässig ist, werden blockiert.
...
- [Hochladen einer benutzerdefinierten VM-VHD in Azure](upload-generalized-managed.md)
...
- [Integrieren von verwalteten Azure-Identitäten](../../azure-app-configuration/howto-integrate-azure-managed-service-identity.md)
...
**Leitfaden**: Verwenden Sie Microsoft Antimalware für virtuelle Azure Windows-Computer, um Ihre Ressourcen kontinuierlich zu überwachen und zu schützen.
...
**Leitfaden**: Wenn Sie verwaltete Azure-Datenträger mit Azure Backup sichern, werden ruhende VMs mit der Speicherdienstverschlüsselung (Storage Service Encryption, SSE) verschlüsselt. Azure Backup kann auch virtuelle Azure-Computer sichern, die mit Azure Disk Encryption verschlüsselt wurden. Azure Disk Encryption kann in BitLocker-Verschlüsselungsschlüssel (BEK) integriert werden, die in einem Schlüsseltresor als Geheimnisse geschützt werden. Azure Disk Encryption kann auch in Azure Key Vault-Schlüssel für die Schlüsselverschlüsselung (Key Encryption Keys, KEK) integriert werden. Aktivieren Sie das vorläufige Löschen in Key Vault, um Schlüssel vor dem versehentlichen oder böswilligen Löschen zu schützen.

- [Vorläufiges Löschen für VMs](../../backup/soft-delete-virtual-machines.md)

- [Übersicht über die Azure Key Vault-Funktion für vorläufiges Löschen](/azure/key-vault/general/overview-soft-delete)
