...
Sie können die Schreibbeschleunigung mithilfe der [Azure CLI](/cli/azure/) aktivieren.

Um die Schreibbeschleunigung auf einem vorhandenen Datenträger zu aktivieren, verwenden Sie [az vm update](/cli/azure/vm#az_vm_update). Sie können die folgenden Beispiele verwenden, wenn Sie diskName, VMName und ResourceGroup durch Ihre eigenen Werte ersetzen:`az vm update -g group1 -n vm1 -write-accelerator 1=true`

Um einen Datenträger mit Schreibbeschleunigung anzufügen, verwenden Sie [az vm disk attach](/cli/azure/vm/disk#az_vm_disk_attach). Dabei können Sie das folgende Beispiel verwenden, wenn Sie Ihre eigenen Werte einsetzen:`az vm disk attach -g group1 -vm-name vm1 -disk d1 --enable-write-accelerator`

Um die Schreibbeschleunigung zu deaktivieren, verwenden Sie [az vm update](/cli/azure/vm#az_vm_update) und legen die Eigenschaften auf „false“ fest:`az vm update -g group1 -n vm1 -write-accelerator 0=false 1=false`
