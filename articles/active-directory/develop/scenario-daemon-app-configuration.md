...
## <a name="microsoft-libraries-supporting-daemon-apps"></a>Microsoft-Bibliotheken unterstützende Daemon-Apps
...
Konfigurationsparameter für das [Node.js-Daemon-Beispiel](https://github.com/Azure-Samples/ms-identity-javascript-nodejs-console/) sind in einer *ENV*-Datei enthalten:
...
Installieren Sie die Pakete einfach durch Ausführen von `npm install` in dem Ordner, in dem sich die Datei *package.json* befindet. Importieren Sie anschließend das Paket **msal-node**:
...
Die Initialisierung mit Zertifikaten wird von der Beispielanwendung momentan nicht implementiert.
...
Die Initialisierung mit Assertionen wird von der Beispielanwendung momentan nicht implementiert.
...
Fahren Sie mit dem nächsten Artikel in diesem Szenario fort: [Abrufen eines Tokens für die App](./scenario-daemon-acquire-token.md?tabs=nodejs).
