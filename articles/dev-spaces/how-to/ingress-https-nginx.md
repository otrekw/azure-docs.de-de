...
Öffnen Sie [values.yaml][values-yaml], und ersetzen Sie alle Vorkommen von *<REPLACE_ME_WITH_HOST_SUFFIX>* durch *nginx.MY_CUSTOM_DOMAIN*, wobei Sie für *MY_CUSTOM_DOMAIN* Ihre Domäne verwenden. Ersetzen Sie zudem *kubernetes.io/ingress.class: nginx-azds  # Dev Spaces-specific* durch *kubernetes.io/ingress.class: nginx # benutzerdefinierter Eingang*. Nachstehend finden Sie ein Beispiel für eine aktualisierte `values.yaml`-Datei:
