...
4. Fügen Sie nun auf die gleiche Weise einen weiteren Back-End-Pool mit dem Namen *fabrikamPool* hinzu.
...
3. Eine Routingregel erfordert einen Listener. Geben Sie im Fenster **Routingregel hinzufügen** auf der Registerkarte **Listener** die folgenden Werte für den Listener ein:
    ...
    - **Regelname**: *contosoRule*.
    - **Listenername**: *contosoListener*.
...
7. Wählen Sie **Routingregel hinzufügen** aus, und fügen Sie eine ähnliche Regel, einen Listener, ein Back-End-Ziel und eine HTTP-Einstellung für Fabrikam hinzu.
...
3. Geben Sie auf der Registerkarte **Grundlagen** die folgenden Werte für die VM-Einstellungen ein:
    ...
    - **Region**: Wählen Sie dieselbe Region aus, die Sie zuvor bereits verwendet haben.
...
4. Legen Sie auf der Registerkarte **Verwaltung** die Option **Startdiagnose** auf **Deaktivieren** fest. Übernehmen Sie für die anderen Einstellungen die Standardwerte, und klicken Sie auf **Bewerten + erstellen**.
...
5. Wählen Sie unter **Ziel** in der Dropdownliste die VM-Netzwerkschnittstelle **contosoVM** aus.
...
2. Kopieren Sie die IP-Adresse, und verwenden Sie sie als Wert für neue Einträge in Ihrer `hosts`-Datei.
1. Öffnen Sie auf dem lokalen Computer eine Administratoreingabeaufforderung, und navigieren Sie zu `c:\Windows\System32\drivers\etc`.
1. Öffnen Sie die `hosts`-Datei, und fügen Sie die folgenden Einträge hinzu, wobei `x.x.x.x` die öffentliche IP-Adresse des Anwendungsgateways ist:
   ```dos
...
1. Führen Sie die folgenden Befehle aus, um die Änderungen an der Hostdatei zu laden und anzuzeigen:
   ```dos
...
1. Geben Sie einen Domänennamen in die Adressleiste Ihres Browsers ein. Beispiel: `http://www.contoso.com`.

    ![Testen der Contoso-Website im Anwendungsgateway](./media/create-multiple-sites-portal/application-gateway-iistest.png)

2. Ändern Sie die Adresse in die andere Domäne. Die Ausgabe sollte in etwa wie folgt aussehen:
...
So stellen Sie die Hostdatei wieder her:
1. Löschen Sie die Zeilen `www.contoso.com` und `www.fabrikam.com` aus der Hostdatei, und führen Sie `ipconfig/registerdns` und `ipconfig/flushdns` in der Eingabeaufforderung aus.
