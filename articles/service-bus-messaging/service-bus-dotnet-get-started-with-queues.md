...
1. Wählen Sie **Durchsuchen**. Suchen Sie nach dem Element **[Microsoft.Azure.ServiceBus](https://www.nuget.org/packages/Microsoft.Azure.ServiceBus/)** , und wählen Sie es aus.
...
1. Ersetzen Sie den Standardinhalt von `Main()` durch die folgende Codezeile:

    ```csharp
    MainAsync().GetAwaiter().GetResult();
    ```

1. Fügen Sie direkt nach `Main()` die folgende asynchrone `MainAsync()`-Methode hinzu, die die Methode für den Nachrichtenversand aufruft:
...
1. Fügen Sie direkt nach `Main()` die folgende asynchrone `MainAsync()`-Methode hinzu, die die `RegisterOnMessageHandlerAndReceiveMessages()`-Methode aufruft:
