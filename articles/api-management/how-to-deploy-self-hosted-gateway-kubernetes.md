...
ms.locfileid: "82204831"
---
# <a name="deploy-self-hosted-gateway-to-kubernetes"></a>Bereitstellen eines selbstgehosteten Gateways für Kubernetes

In diesem Artikel werden die Schritte für die Bereitstellung einer selbstgehosteten Gatewaykomponente von Azure API Management für einen Kubernetes-Cluster beschrieben.
...
## <a name="deploy-to-kubernetes"></a>Bereitstellen für Kubernetes
...
6. Klicken Sie auf den Link zur Datei **<Gatewayname>.yml**, und laden Sie die YAML-Datei herunter.
...
10. Führen Sie den nachstehenden Befehl aus, um zu prüfen, ob die Bereitstellung erfolgreich war. Beachten Sie, dass es einige Zeit dauern kann, bis alle Objekte erstellt sind und die Pods initialisiert wurden.
```console
kubectl get deployments
NAME             READY   UP-TO-DATE   AVAILABLE   AGE
<gateway-name>   1/1     1            1           18s
```
11. Führen Sie den nachstehenden Befehl aus, um zu prüfen, ob der Dienst erfolgreich erstellt wurde. Beachten Sie, dass sich die IP-Adressen und Ports Ihrer Dienste unterscheiden.
```console
...
13. **Status** wird mit einem grünen Häkchen angezeigt. Es folgt die Anzahl der Knoten, die mit der in der YAML-Datei angegebenen Anzahl von Replikaten übereinstimmt. Dadurch wird bestätigt, dass die bereitgestellten selbstgehosteten Gatewaypods erfolgreich mit dem API Management-Dienst kommunizieren und einen regelmäßigen „Takt“ haben.
...
Ohne ein gültiges Zugriffstoken ist das selbstgehostete Gateway nicht in der Lage, am Endpunkt der Konfigurationsdaten des zugehörigen API Management-Diensts auf die Konfiguration zuzugreifen und diese herunterzuladen. Das Zugriffstoken ist maximal 30 Tage gültig. Es muss neu generiert und der Cluster entweder manuell oder mittels Automatisierung mit einem neuen Token konfiguriert werden, bevor es abläuft. Wenn Sie die Tokenaktualisierung automatisieren, generieren Sie über diesen [Vorgang](https://docs.microsoft.com/rest/api/apimanagement/2019-12-01/gateway/generatetoken) der Verwaltungs-API ein neues Token. Folgen Sie diesem [Link](https://kubernetes.io/docs/concepts/configuration/secret) zu Informationen zur Verwaltung von Kubernetes-Geheimnissen.

### <a name="namespace"></a>Namespace
Kubernetes-[Namespaces](https://kubernetes.io/docs/concepts/overview/working-with-objects/namespaces/) helfen bei der Aufteilung eines einzelnen Clusters unter mehreren Teams, Projekten oder Anwendungen. Namespaces bieten einen Bereich für Ressourcen und Namen und können einem Ressourcenkontingent und Zugriffssteuerungsrichtlinien zugeordnet werden.
Im Azure-Portal bereitgestellte Befehle generieren selbstgehostete Gatewayressourcen im **standardmäßigen** Namespace, der automatisch erstellt wird, in jedem Cluster vorhanden ist und nicht gelöscht werden kann.
...
Die minimale Anzahl der für die Produktion geeigneten Replikate ist zwei.
...
Es ist unmöglich, die Menge an CPU- und Arbeitsspeicherressourcen pro Container und die Anzahl der Replikate, die zur Unterstützung einer bestimmten Workload erforderlich sind, zuverlässig vorherzusagen und zu empfehlen, da viele Faktoren eine Rolle spielen, wie z. B:

- Spezifische Hardware, auf der der Cluster ausgeführt wird
- Präsenz und Virtualisierungstyp
...
Wir empfehlen, die Ressourcenanforderung auf 2 Kerne und 2 GiB als Ausgangspunkt festzulegen, einen Lasttest durchzuführen und basierend auf den Ergebnissen ein Auf-/Hoch- bzw. Herunter-/Abskalieren vorzunehmen.

### <a name="container-image-tag"></a>Tag für Containerimage
Die im Azure-Portal bereitgestellte YAML-Datei verwendet das Tag **latest**, das stets auf die neueste Version des Containerimages des selbstgehosteten Gateways verweist.
...
Folgen Sie diesem [Link](https://mcr.microsoft.com/v2/azure-api-management/gateway/tags/list), um die vollständige Liste der verfügbaren Tags einzusehen.
...
Die im Azure-Portal bereitgestellte YAML-Datei wendet die Standardrichtlinie [**ClusterFirst**](https://kubernetes.io/docs/concepts/services-networking/dns-pod-service/#pod-s-dns-policy) an, die bewirkt, dass vom Cluster-DNS nicht aufgelöste Namensauflösungsanforderungen an den vom Knoten geerbten vorgelagerten DNS-Server weitergeleitet werden.

Folgen Sie diesem [Link](https://kubernetes.io/docs/concepts/services-networking/dns-pod-service), um mehr über die Namensauflösung in Kubernetes zu erfahren, und erwägen Sie, die [DNS-Richtlinie](https://kubernetes.io/docs/concepts/services-networking/dns-pod-service/#pod-s-dns-policy) oder [DNS-Konfiguration](https://kubernetes.io/docs/concepts/services-networking/dns-pod-service/#pod-s-dns-config) entsprechend Ihrer Einrichtung anzupassen.

### <a name="configuration-backup"></a>Sicherung der Konfiguration
Folgen Sie diesem [Link](self-hosted-gateway-overview.md#connectivity-to-azure), um mehr über das Verhalten selbstgehosteter Gateways bei einem vorübergehenden Ausfall der Verbindung mit Azure zu erfahren.
Konfigurieren Sie ein lokales Speichervolume für den selbstgehosteten Gatewaycontainer, damit dieser eine Sicherungskopie der zuletzt heruntergeladenen Konfiguration speichert und diese bei Ausfall der Verbindung nach einem Neustart verwenden kann. Der Einbindungspfad des Volumes muss <code>/apim/config</code> sein. Ein Beispiel finden Sie [hier](https://github.com/Azure/api-management-self-hosted-gateway/blob/master/examples/self-hosted-gateway-with-configuration-backup.yaml).
Folgen Sie diesem [Link](https://kubernetes.io/docs/concepts/storage/volumes/), um mehr über die Speicherung in Kubernetes zu erfahren.

### <a name="local-logs-and-metrics"></a>Lokale Protokolle und Metriken
Das selbst gehostete Gateway sendet Telemetrie an [Azure Monitor](api-management-howto-use-azure-monitor.md) und [Azure Application Insights](api-management-howto-app-insights.md) gemäß Konfigurationseinstellungen im zugehörigen API Management-Dienst.
