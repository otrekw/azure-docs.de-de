...
ms.locfileid: "76850737"
---
# <a name="migrate-users-to-azure-active-directory-b2c"></a>Migrieren von Benutzern zu Azure Active Directory B2C
...
![Teil der Datei „UsersData.json“ mit JSON-Blöcken von zwei Benutzern](./media/user-migration/pre-migration-data-file.png)
...
![Eingabeaufforderungsfenster mit Ausgabe des Befehls „UserMigration.exe“](./media/user-migration/pre-migration-demo.png)
...
- Verwenden Sie diese Beispielanwendung, um einen Benutzer anhand der E-Mail-Adresse für die Anmeldung abzurufen:
   ...
   1. Öffnen Sie die Datei „UserProfile.json“ in einem JSON-Editor, um die Informationen des Benutzers anzuzeigen.
      ...
      ![Datei „UserProfile.json“ im Visual Studio Code-Editor geöffnet](./media/user-migration/pre-migration-get-by-email2.png)
...
Führen Sie die folgenden Schritte aus, um den Link zu Ihrer Richtlinie für die Kennwortzurücksetzung abzurufen. Bei diesem Verfahren wird vorausgesetzt, dass Sie zuvor eine [benutzerdefinierte Richtlinie](custom-policy-get-started.md) für die Kennwortzurücksetzung erstellt haben.
...
1. Kopieren Sie die im Textfeld **Endpunkt für sofortige Ausführung** angezeigte URL, und senden Sie sie anschließend an Ihre Benutzer.
    ...
    ![Seite „Richtlinie für die Kennwortzurücksetzung“ mit „Endpunkt für sofortige Ausführung“ hervorgehoben](./media/user-migration/pre-migration-policy-uri.png)
...
Nachdem Sie das technische Profil für Ihre RESTful-API definiert haben, konfigurieren Sie das vorhandene technische Profil `SelfAsserted-LocalAccountSignin-Email` so, dass es zusätzlich Ihr technisches Profil der REST-API aufruft, indem es in Ihrer *TrustFrameworkExtensions.xml*-Datei außer Kraft gesetzt wird:
...
Ändern Sie dann die `Id` des technischen Profils `LocalAccountSignIn` in `LocalAccountUserMigration`.
...
1. Geben Sie die Anmeldeinformationen eines migrierten Benutzers ein, und wählen Sie dann **Anmelden** aus. Ihre REST-API sollte die folgende Fehlermeldung auslösen:
    ...
    ![Anmeldeseite für die Registrierung mit der Fehlermeldung zum Ändern des Kennworts](./media/user-migration/pre-migration-error-message.png)
...
1. Wählen Sie **Speichern** aus.
    ...
    ![Konfigurationsseite „Diagnoseprotokolle“ im Azure-Portal](./media/user-migration/pre-migration-diagnostic-logs.png)
