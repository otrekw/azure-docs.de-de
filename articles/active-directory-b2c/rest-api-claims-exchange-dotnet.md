---
title: Integrieren von REST-API-Anspruchsaustauschvorgängen in eine User Journey
titleSuffix: Azure AD B2C
description: Integrieren von REST-API-Anspruchsaustauschvorgängen in Ihre Azure AD B2C-User Journey zur Validierung der Benutzereingabe.
...
Führen Sie die Schritte im Artikel [Erste Schritte mit benutzerdefinierten Richtlinien](custom-policy-get-started.md) aus.
...
1. Wählen Sie im Fenster **Neues Projekt** die Optionen **Visual C#**  > **Web** > **ASP.NET-Webanwendung (.NET Framework)** aus.
1. Geben Sie im Feld **Name** einen Namen für die Anwendung an (beispielsweise *Contoso.AADB2C.API*), und wählen Sie dann **OK** aus.

    ![Erstellen eines neuen Visual Studio-Projekts in Visual Studio](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-create-project.png)

1. Wählen Sie im Fenster **Neue ASP.NET-Webanwendung** eine **Web-API**- oder **Azure-API-App**-Vorlage aus.

    ![Auswählen einer Web-API-Vorlage in Visual Studio](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-select-web-api.png)
...
1. Klicken Sie im Projektmappen-Explorer mit der rechten Maustaste auf den Ordner **Modelle**, und wählen Sie dann **Hinzufügen** und anschließend **Klasse** aus.

    ![Hinzufügen eines ausgewählten Klassenmenüelements in Visual Studio](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-add-model.png)

1. Geben Sie der Klasse den Namen `InputClaimsModel`, und fügen Sie der `InputClaimsModel`-Klasse die folgenden Eigenschaften hinzu:
...
1. Erstellen Sie das neue Modell `OutputClaimsModel`, und fügen Sie der `OutputClaimsModel`-Klasse die folgenden Eigenschaften hinzu:
...
1. Erstellen Sie ein weiteres Modell (`B2CResponseContent`), das Sie verwenden, um die Fehlermeldungen der Eingabevalidierung auszulösen. Fügen Sie der `B2CResponseContent`-Klasse die folgenden Eigenschaften hinzu, geben Sie die fehlenden Verweise an, und speichern Sie die Datei:
...
1. Klicken Sie im Projektmappen-Explorer mit der rechten Maustaste auf den Ordner **Controller**, und wählen Sie dann **Hinzufügen** und **Controller** aus.
    ...
    ![Hinzufügen eines neuen Controllers in Visual Studio](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-add-controller-1.png)

1. Wählen Sie im Dialogfeld **Gerüst hinzufügen** die Option **Web-API-Controller – Leer** und dann **Hinzufügen** aus.

    ![Auswählen der Option „Web API 2-Controller – Leer“ in Visual Studio](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-add-controller-2.png)

1. Geben Sie dem Controller im Fenster **Controller hinzufügen** den Namen **IdentityController**, und wählen Sie dann **Hinzufügen** aus.

    ![Eingeben des Controllernamens in Visual Studio](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-add-controller-3.png)

    Im Rahmen des Gerüstbauvorgangs wird im Ordner *Controller* die Datei *IdentityController.cs* erstellt.

1. Wenn die Datei *IdentityController.cs* noch nicht geöffnet ist, doppelklicken Sie darauf, und ersetzen Sie dann den Code in der Datei durch folgenden Code:
...
1. Klicken Sie im Projektmappen-Explorer mit der rechten Maustaste auf das Projekt **Contoso.AADB2C.API**, und wählen Sie dann **Veröffentlichen** aus.
    ...
    ![Veröffentlichen in Microsoft Azure App Service mit Visual Studio](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-publish-to-azure-1.png)

1. Wählen Sie im Fenster **Veröffentlichen** die Option **Microsoft Azure App Service** und anschließend **Veröffentlichen** aus.

    ![Erstellen einer neuen Microsoft Azure App Service-Instanz mit Visual Studio](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-publish-to-azure-2.png)
    ...
    > [!TIP]
    > Weitere Informationen zum Veröffentlichen finden Sie unter [Erstellen von ASP.NET-Web-Apps in Azure](../app-service/app-service-web-get-started-dotnet-framework.md).

1. Geben Sie im Feld **Web-App-Name** einen eindeutigen App-Namen ein (gültige Zeichen sind a–z, A–Z, 0–9 und der Bindestrich [-]). Die URL der Web-App lautet: http://<App-Name>.azurewebsites.NET, wobei *App-Name* der Name Ihrer Web-App ist. Sie können den automatisch generierten Namen übernehmen. Dieser ist eindeutig.

    ![Konfigurieren der App Service-Eigenschaften](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-publish-to-azure-3.png)
...
1. Wiederholen Sie den vorherigen Schritt mit der Datei „SignUpOrSignIn.xml“.
...
1. Wählen Sie **Azure AD B2C-Einstellungen** aus, und navigieren Sie dann zu **Framework für die Identitätsfunktion**.
    ...
    > [!NOTE]
    > **Jetzt ausführen** macht erforderlich, dass mindestens eine Anwendung im Mandanten vorab registriert wird. Informationen zum Registrieren von Anwendungen finden Sie in den Artikeln [Erste Schritte](tutorial-create-tenant.md) bzw. [Anwendungsregistrierung](tutorial-register-applications.md) zu Azure AD B2C.
...
2. Öffnen Sie **B2C_1A_signup_signin**. Dies ist die benutzerdefinierte Richtlinie der vertrauenden Seite (Relying Party, RP), die Sie hochgeladen haben. Wählen Sie anschließend **Jetzt ausführen** aus.
    ...
    ![Die benutzerdefinierte Richtlinienseite „B2C_1A_signup_signin“ im Azure-Portal](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-run.png)
...
3. Testen Sie den Prozess, indem Sie **Test** in das Feld **Vorname** eingeben.
    ...
    ![Testen der Eingabevalidierung für den Vornamen auf der Registrierungs- oder Anmeldeseite](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-test.png)
...
Im nächsten Schritt muss Ihre RESTful-API per Standard- oder Clientzertifikatauthentifizierung geschützt werden. Informationen zum Schützen Ihrer APIs finden Sie in den folgenden Artikeln:
...
Informationen zu allen in einem technischen RESTful-Profil verfügbaren Elementen finden Sie unter [Referenz: Technisches RESTful-Profil](restful-technical-profile.md).
