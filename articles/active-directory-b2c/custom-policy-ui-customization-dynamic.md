...
Dieser Artikel konzentriert sich auf das Anpassen der Azure AD B2C-Benutzeroberfläche mit *dynamischen Inhalten* unter Verwendung benutzerdefinierter Richtlinien. Informationen zum Einstieg finden Sie unter [Benutzeroberflächenanpassung in einer benutzerdefinierten Richtlinie](custom-policy-ui-customization.md).

>[!NOTE]
>Im Artikel [Azure Active Directory B2C: Konfigurieren der Benutzeroberflächenanpassung in einer benutzerdefinierten Richtlinie](custom-policy-ui-customization.md) werden die folgenden Grundlagen erläutert:
...
Im Artikel [Azure Active Directory B2C: Konfigurieren der Benutzeroberflächenanpassung in einer benutzerdefinierten Richtlinie](custom-policy-ui-customization.md) laden Sie HTML5-Dateien in Azure Blob Storage hoch. Diese HTML5-Dateien sind statisch und stellen für jede Anforderung den gleichen HTML-Inhalt dar.
...
3. Benennen Sie die Anwendung (z.B. *Contoso.AADB2C.UI*), und wählen Sie dann **OK** aus.
    ...
    ![Erstellen eines neuen Visual Studio-Projekts](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-create-project1.png)

4. Wählen Sie die Vorlage **Webanwendung** aus.

5. Legen Sie die Authentifizierung auf **Keine Authentifizierung** fest.

    ![Auswählen der Vorlage „Webanwendung“](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-create-project2.png)
...
1. Klicken Sie mit der rechten Maustaste auf den Ordner „Home“ unter „Views“, und klicken Sie dann auf **Hinzufügen** > **Neues Element**.
    ...
    ![Menüelement „Neues Element hinzufügen“ in Visual Studio](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-add-view1.png)
...
5. Wählen Sie **Hinzufügen**.
    ...
    ![Dialogfeld „Neues Element hinzufügen“ in Visual Studio mit hervorgehobener MVC-Ansichtsseite](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-add-view2.png)
...
10. Kopieren Sie den Inhalt der Datei, und fügen Sie ihn unterhalb der Layout-Definition ein. Der Code sollte wie folgt aussehen:
    ...
    ![Datei „unified.cshtml“ nach dem Hinzufügen der HTML5-Vorlage](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-edit-view1.png)
...
![img-Element mit benutzerdefiniertem src-Wert für background_background_image](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-add-static-background.png)
...
1. Öffnen Sie **Controllers\HomeController.cs**, und fügen Sie die folgende Methode hinzu:
    ...
    ![Ändern des Controllers zum Ausgeben der Ansicht](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-controller-view.png)
...
1. Klicken Sie im **Projektmappen-Explorer** mit der rechten Maustaste auf das Projekt **Contoso.AADB2C.UI**, und wählen Sie dann **Veröffentlichen** aus.
    ...
    ![Veröffentlichen in Microsoft Azure App Service](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-publish1.png)

2. Wählen Sie die Kachel **Microsoft Azure App Service** und anschließend **Veröffentlichen** aus.

    ![Erstellen eines neuen Microsoft Azure App Service](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-publish2.png)
...
4. Wählen Sie **Erstellen** aus, um mit der Erstellung der Azure-Ressourcen zu beginnen.
    ...
    ![Bereitstellen der App Service-Eigenschaften](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-publish3.png)
...
1. Wählen Sie im [Azure-Portal](https://portal.azure.com/) die Option **App Services** und anschließend den Namen Ihrer API-App aus.
    ...
    ![Auswählen der API-App im Azure-Portal](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-CORS1.png)

2. Wählen Sie im Abschnitt **Einstellungen** unter dem Abschnitt **API** die Option **CORS** aus.

    ![Hervorgehobenes CORS-Menüelement im App Service-Menü im Azure-Portal](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-CORS2.png)
...
4. Wählen Sie **Speichern** aus.
    ...
    ![CORS-Einstellungsseite mit hervorgehobenem Sternchen unter „Zulässige Ursprünge“](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-CORS3.png)
...
6. Ändern Sie den Wert von `LoadUri` von _~/tenant/default/unified_ in _https://<App-Name>.azurewebsites.net/home/unified_.
    ...
    ![XML-Beispielcodeausschnitt mit hervorgehobenem LoadUri-Element](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-content-definition.png)
...
2. Öffnen Sie **B2C_1A_signup_signin**. Dies ist die benutzerdefinierte Richtlinie der vertrauenden Seite (Relying Party, RP), die Sie hochgeladen haben. Wählen Sie anschließend **Jetzt ausführen** aus.
    ...
    ![Ihre Registrierungs- oder Anmelderichtlinie](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-demo1.png)
...
2. Suchen Sie das `<img>`-Element mit der ID `background_background_image`, und ersetzen Sie den `src`-Wert durch `@ViewData["background"]`.
    ...
    ![img-Element mit hervorgehobenem src-Wert ](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-add-dynamic-background.png)
...
5. Fügen Sie dem URI den Abfragezeichenfolgen-Parameter _campaignId_ hinzu. Fügen Sie z.B. `&campaignId=hawaii` hinzu, wie in der folgenden Abbildung dargestellt:
    ...
    ![URI mit hervorgehobenem campaignId-Abfragezeichenfolgenparameter](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-campaignId-param.png)

6. Drücken Sie die **EINGABETASTE**, um das Hawaii-Hintergrundbild anzuzeigen.

    ![Registrierungs- bzw. Anmeldeseite mit Hawaiibild als benutzerdefinierter Hintergrund](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-demo2.png)

7. Ändern Sie den Wert in *Tokyo*, und drücken Sie dann die **EINGABETASTE**.
    Im Browser wird der Tokio-Hintergrund angezeigt.

    ![Registrierungs- bzw. Anmeldeseite mit Tokiobild als benutzerdefinierter Hintergrund](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-demo3.png)
