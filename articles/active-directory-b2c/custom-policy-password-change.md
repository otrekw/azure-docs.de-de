...
In Azure Active Directory B2C (Azure AD B2C) können Sie es mit einem lokalen Konto angemeldeten Benutzern ermöglichen, ihr Kennwort zu ändern, ohne die Authentizität durch E-Mail-Überprüfung nachweisen zu müssen. Wenn die Sitzung abläuft, bevor der Benutzer den Kennwortänderungsflow erreicht, wird er aufgefordert, sich erneut anzumelden. In diesem Artikel wird beschrieben, wie Sie die Kennwortänderung in [benutzerdefinierten Richtlinien](custom-policy-overview.md) konfigurieren. Es ist auch möglich, die [Self-Service-Kennwortzurücksetzung](user-flow-self-service-password-reset.md) für Benutzerflows zu konfigurieren.
