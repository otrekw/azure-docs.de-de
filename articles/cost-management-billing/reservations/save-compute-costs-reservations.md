...
![Beispiel für die Kaufauswahl einer VM-Reservierung](./media/save-compute-costs-reservations/select-product-to-purchase.png)
...
![Beispiel für eine Änderung des Reservierungsbereichs](./media/save-compute-costs-reservations/rescope-reservation-resource-group.png)
...
Wenn Sie Kunde unter einem Enterprise Agreement oder einer Microsoft-Kundenvereinbarung sind, können Sie die Reservierungsnutzung auch mithilfe von [APIs](reservation-apis.md#see-reservation-usage) und aus Ihren [Nutzungsdaten](understand-reserved-instance-usage-ea.md#common-cost-and-usage-tasks) abrufen.
...
Ein Reservierungsbereich wird nicht automatisch aktualisiert, wenn Sie die Ressourcengruppe aus einem Abonnement in ein anderes verschieben. Der Bereich wird nicht aktualisiert, wenn Sie die Ressourcengruppe löschen. Sie müssen den [Bereich der Reservierung neu festlegen](manage-reserved-vm-instance.md#change-the-reservation-scope). Andernfalls wird die Reservierung nicht ausgenutzt.
