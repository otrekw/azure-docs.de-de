---
title: Versionshinweise zur Vorschauversion von Azure Stack Edge | Microsoft-Dokumentation
...
ms.locfileid: "89322397"
---
# <a name="azure-stack-edge-with-gpu-preview-release-notes"></a>Versionshinweise zur Vorschauversion von Azure Stack Edge mit GPU

In den folgenden Versionshinweisen werden die schwerwiegenden offenen und die behobenen Probleme für das Vorschaurelease 2008 für Azure Stack Edge-Geräte mit GPU beschrieben.

Die Versionshinweise werden fortlaufend aktualisiert, und wenn schwerwiegende Probleme festgestellt werden, die eine Problemumgehung erfordern, werden sie hinzugefügt. Lesen Sie vor der Bereitstellung Ihres Azure Stack Edge-Geräts die Informationen in den Versionshinweisen sorgfältig durch.

Das Release 2008 von **Azure Stack Edge** entspricht der folgenden Softwareversion:

- **2.1.1328.1904**

## <a name="whats-new"></a>Neues

Die folgenden neuen Features wurden im Release 2008 von Azure Stack Edge hinzugefügt:

- **Speicherklassen:** Im vorherigen Release konnten Sie Speicher nur statisch über SMB- oder NFS-Freigaben für zustandsbehaftete Anwendungen bereitstellen, die im Kubernetes-Cluster auf dem Azure Stack Edge-Gerät bereitgestellt wurden. In diesem Release wurden Speicherklassen hinzugefügt, mit denen Speicher dynamisch bereitgestellt werden kann. Weitere Informationen finden Sie unter [Kubernetes-Speicherverwaltung auf dem Azure Stack Edge-GPU-Gerät](azure-stack-edge-gpu-kubernetes-storage.md#dynamicprovisioning). 
- **Kubernetes-Dashboard mit Metrics Server:** In diesem Release wird ein Kubernetes-Dashboard mit dem Metrics Server-Add-On hinzugefügt. Sie können das Dashboard verwenden, um eine Übersicht über die auf dem Azure Stack Edge-Gerät ausgeführten Anwendungen, den Status von Kubernetes-Clusterressourcen und eventuelle auf dem Gerät aufgetretene Fehler anzuzeigen. Mit dem Metrics Server-Add-On wird die CPU- und Speicherauslastung der Kubernetes-Ressourcen auf dem Gerät aggregiert. Weitere Informationen finden Sie unter [Überwachen des Azure Stack Edge-GPU-Geräts über das Kubernetes-Dashboard](azure-stack-edge-gpu-monitor-kubernetes-dashboard.md).
- **Azure Arc for Azure Stack Edge:** Ab diesem Release können Sie Anwendungsworkloads auf dem Azure Stack Edge-Gerät über Azure Arc bereitstellen. Azure Arc ist ein Hybridverwaltungstool, mit dem Sie Anwendungen in Ihren Kubernetes-Clustern bereitstellen können. Weitere Informationen finden Sie unter [Bereitstellen von Workloads über Azure Arc auf dem Azure Stack Edge-Gerät](azure-stack-edge-gpu-deploy-arc-kubernetes-cluster.md).  

## <a name="known-issues"></a>Bekannte Probleme 

Die folgende Tabelle enthält eine Zusammenfassung der bekannten Probleme für das Azure Stack Edge-Gerät.

| Nein. | Funktion | Problem | Problemumgehung/Kommentare |
| --- | --- | --- | --- |
| **1.** |Azure Stack Edge + Azure SQL | Zum Erstellen einer SQL-Datenbank ist Administratorzugriff erforderlich.   |Führen Sie statt der Schritte 1 und 2 unter [https://docs.microsoft.com/azure/iot-edge/tutorial-store-data-sql-server#create-the-sql-database](https://docs.microsoft.com/azure/iot-edge/tutorial-store-data-sql-server#create-the-sql-database) die folgenden Schritte aus. <ul><li>Aktivieren Sie in der lokalen Benutzeroberfläche des Geräts die Computeschnittstelle. Wählen Sie **Compute > Portnr. > Für Compute aktivieren > Anwenden** aus.</li><li>Laden Sie `sqlcmd` von https://docs.microsoft.com/sql/tools/sqlcmd-utility auf den Clientcomputer herunter. </li><li>Stellen Sie eine Verbindung mit der IP-Adresse der Computeschnittstelle (dem aktivierten Port) her, und fügen Sie am Ende der Adresse „,1401“ hinzu.</li><li>Der endgültige Befehl sieht wie folgt aus: sqlcmd -S {IP-Adresse-der-Schnittstelle},1401 -U SA -P "Starkes!Kennw0rt".</li>Danach sollten die Schritte 3 und 4 aus der aktuellen Dokumentation identisch sein. </li></ul> |
...
|**8.**|Kubernetes-Cluster|Kubernetes wird in Azure Stack Edge nur mit Helm Version 3 oder höher unterstützt. Weitere Informationen finden Sie unter [Frequently asked questions: Removal of Tiller](https://v3.helm.sh/docs/faq/) (Häufig gestellte Fragen: Entfernen von Tiller).|
|**9.**|Azure Arc + Azure Stack Edge|Azure Arc-Bereitstellungen werden nicht unterstützt, wenn auf dem Azure Stack Edge-Gerät ein Webproxy konfiguriert ist.||
...
- [Vorbereiten der Bereitstellung von Azure Stack Edge-Geräten mit GPU](azure-stack-edge-gpu-deploy-prep.md)
