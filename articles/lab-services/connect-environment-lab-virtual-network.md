---
title: Verbinden von Umgebungen mit dem VNet eines Labs in Azure DevTest Labs | Microsoft-Dokumentation
description: Erfahren Sie, wie Sie in Azure DevTest Labs eine Umgebung (z.B. einen Service Fabric-Cluster) mit dem virtuellen Netzwerk Ihres Labs verbinden können.
...
## <a name="sample-environment-that-uses-lab-vnet"></a>Beispielumgebung, in der das VNet im Lab verwendet wird
Hier ist eine einfache Umgebungsvorlage, über die eine Verbindung mit dem Subnetz des Labs hergestellt wird. Bei diesem Beispiel stellt der Parameter `DTLSubnetId` die ID des Subnetzes dar, in dem sich das Lab befindet. Sie ist `$(LabSubnetId)` zugeordnet, was von DevTest Labs automatisch in die ID des Subnetzes des Labs aufgelöst wird. Die Eigenschaft **subnet** der **Netzwerkschnittstelle** der VM wird in dieser Definition auf `DTLSubnetId` festgelegt, sodass sie dem gleichen Subnetz beitritt. 
