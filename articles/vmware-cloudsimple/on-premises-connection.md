---
title: 'Azure VMware Solutions (AVS): lokale Verbindung mithilfe von ExpressRoute'
description: Beschreibt, wie eine lokale Verbindung mithilfe von ExpressRoute aus dem AVS-Regionsnetzwerk angefordert wird
author: sharaths-cs
ms.author: b-shsury
ms.date: 08/14/2019
ms.topic: article
...
ms.locfileid: "77019620"
---
# <a name="connect-from-on-premises-to-avs-using-expressroute"></a>Herstellen einer Verbindung aus der lokalen Umgebung mit AVS mithilfe von ExpressRoute

Wenn Sie bereits über eine Azure ExpressRoute-Verbindung von einem externen Standort (z. B. einer lokalen Umgebung) mit Azure verfügen, können Sie diese mit Ihrer AVS-Umgebung verbinden. Hierfür können Sie eine Azure-Funktion verwenden, mit der zwei ExpressRoute-Leitungen miteinander verbunden werden können. Diese Methode stellt eine sichere, private Verbindung mit hoher Bandbreite und geringer Latenz zwischen den beiden Umgebungen her.
...
Ein **/29**-Netzwerkadressblock ist erforderlich, um Global Reach-Verbindungen vom lokalen Standort aus herzustellen. Der „/29“-Adressraum wird für das Transitnetzwerk zwischen ExpressRoute-Leitungen verwendet. Zwischen dem Transitnetzwerk und virtuellen Azure-Netzwerken, lokalen Netzwerken oder privaten AVS-Netzwerken darf es keine Überschneidungen geben.

## <a name="prerequisites"></a>Voraussetzungen

* Sie müssen über eine Azure ExpressRoute-Leitung verfügen, bevor Sie die Verbindung zwischen der Leitung und den Netzwerken der privaten AVS-Cloud herstellen können.
...
Wenn Sie eine Verbindung Ihres lokalen Netzwerks mit Ihrem Netzwerk in der privaten AVS-Cloud herstellen, können Sie die private AVS-Cloud auf verschiedene Weise verwenden, einschließlich der folgenden Szenarien:

* Zugreifen auf Ihr privates AVS-Cloudnetzwerk, ohne eine Site-to-Site-VPN-Verbindung zu erstellen
* Verwenden Ihres lokalen Active Directory als Identitätsquelle für Ihre private AVS-Cloud
* Migrieren lokal ausgeführter virtueller Computer zu Ihrer privaten AVS-Cloud
* Verwenden Ihrer privaten AVS-Cloud als Teil einer Lösung für die Notfallwiederherstellung
* Nutzen der lokalen Ressourcen auf den Workload-VMs in Ihrer privaten AVS-Cloud

## <a name="connecting-expressroute-circuits"></a>Verbinden von ExpressRoute-Leitungen

Zum Einrichten der ExpressRoute-Verbindung müssen Sie eine Autorisierung für Ihre ExpressRoute-Leitung erstellen und diese Autorisierungsinformationen in AVS bereitstellen.
...
3. Wählen Sie die ExpressRoute-Leitung aus, die Sie mit Ihrem AVS-Netzwerk verbinden möchten.
...
6. Reichen Sie ein Ticket für die zu erstellende Verbindung beim <a href="https://portal.azure.com/#blade/Microsoft_Azure_Support/HelpAndSupportBlade/newsupportrequest" target="_blank">Support</a> ein.
    ...
    * Abonnement: **Abonnement, in dem der AVS-Dienst bereitgestellt wird**
    ...
    * Sollte der Datenverkehr der privaten AVS-Cloud die Standardroute über ExpressRoute verwenden?

    > [!IMPORTANT]
    > Mithilfe der Standardroute können Sie den gesamten Internetdatenverkehr aus der privaten AVS-Cloud über Ihre lokale Internetverbindung senden. Um die für die private AVS-Cloud konfigurierte Standardroute zu deaktivieren und die Standardroute für die lokale Verbindung zu verwenden, geben Sie die Details im Supportticket an.
