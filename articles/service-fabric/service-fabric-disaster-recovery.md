...
`InstanceCount` gibt für den Dienst die gewünschte Anzahl von Instanzen an, die ausgeführt werden müssen. Wenn eine der Instanzen fehlerhaft ist (oder alle es sind), reagiert Service Fabric durch das automatische Erstellen von Ersatzinstanzen auf anderen Knoten. Dies wird von Service Fabric so lange fortgesetzt, bis wieder die gewünschte Anzahl von Dienstinstanzen erreicht ist.
Ein weiteres Beispiel: Wenn der zustandslose Dienst einen `InstanceCount` von -1 aufweist, sollte auf jedem Knoten im Cluster eine Instanz ausgeführt werden. Wenn bei einigen dieser Instanzen ein Fehler auftreten sollte, erkennt Service Fabric, dass der Dienst nicht den gewünschten Zustand aufweist, und versucht, die Instanzen auf den Knoten, auf denen sie fehlen, zu erstellen.

#### <a name="stateful-services"></a>Zustandsbehaftete Dienste
Es gibt zwei Arten von zustandsbehafteten Diensten:
1. Zustandsbehaftet mit persistentem Zustand
2. Zustandsbehaftet mit nicht persistentem Zustand (Status wird im Arbeitsspeicher gespeichert)
...
Was ist ein Quorum?
Bei einem zustandsbehafteten Dienst werden eingehende Daten zwischen Replikaten (primär und aktiv sekundär) repliziert. Wenn die meisten der Replikate die Daten empfangen, werden die Daten als im Quorum committet angesehen (bei 5 Replikaten gelten 3 als _Quorum_). Dies bedeutet, dass zu jedem Zeitpunkt sichergestellt ist, dass mindestens ein Quorum der Replikate mit den neuesten Daten vorhanden ist. Wenn Replikate fehlerhaft sind (etwa wenn 2 von 5 Replikaten ausfallen), können Sie mithilfe des Quorumwerts berechnen, ob eine Wiederherstellung möglich ist (da weiterhin 3 von 5 Replikaten in Betrieb sind, ist sichergestellt, dass mindestens ein Replikat über die vollständigen Daten verfügt).

Was ist ein Quorumverlust?
Wenn das Quorum der Replikate einen Fehler aufweist, ändert sich der Zustand der Partition in einen Quorumverlust. Nehmen wir an, eine Partition verfügt über 5 Replikate, d. h., mindestens 3 verfügen über vollständige Daten. Wenn das Quorum der Replikate (3 von 5) ausfällt, kann Service Fabric nicht ermitteln, ob die verbleibenden Replikate (2 von 5) über ausreichende Daten für das Wiederherstellen der Partition verfügen.
...
Replikate könnten aufgrund eines vorübergehenden Fehlers zeitweise inaktiv sein. Warten Sie einige Zeit, da Service Fabric versucht, sie wieder zu aktivieren. Wenn Replikate länger als den zu erwartenden Zeitraum nicht verfügbar sind, befolgen Sie die folgenden Schritte zur Problembehandlung.
- Replikate können abstürzen. Überprüfen Sie die Integritätsberichte und Anwendungsprotokolle auf Replikatebene. Erfassen Sie Absturzabbilder, und ergreifen Sie erforderliche Maßnahmen zur Wiederherstellung.
...
- Knoten, auf denen die Replikate gehostet werden, können ausfallen. Starten Sie den zugrunde liegenden virtuellen Computer neu, um die Knoten wieder zu starten.

Es kann Situationen geben, in denen es nicht möglich ist, Replikate wiederherzustellen, z. B. wenn die Laufwerke fehlerhaft sind oder die Computer physisch nicht reagieren. In diesen Fällen muss Service Fabric angewiesen werden, nicht auf die Wiederherstellung der Replikate zu warten.
Verwenden Sie diese Methoden NICHT, wenn ein potenzieller DATENVERLUST durch das erneute Aktivieren des Diensts unbedingt vermieden werden muss! In diesem Fall sollten alle Maßnahmen der Wiederherstellung der physischen Computer gelten.

Die folgenden Schritte können zu Datenverlust führen, wägen Sie sie daher vor der Anwendung gründlich ab:
   
> [!NOTE]
> Aus Sicherheitsgründen dürfen diese Methoden _ausschließlich_ gezielt für bestimmte Partitionen verwendet werden. 
>

- Verwenden Sie `Repair-ServiceFabricPartition -PartitionId` oder die `System.Fabric.FabricClient.ClusterManagementClient.RecoverPartitionAsync(Guid partitionId)`-API. Diese API ermöglicht die Angabe der ID für die Partition, die bei einem Quorumverlust mit einem potenziellen Datenverlust wiederhergestellt werden soll.
- Wenn in Ihrem Cluster häufig Fehler auftreten, die zu einem Quorumverlust von Diensten führen und potenzielle _Datenverluste akzeptabel_ sind, kann die automatische Wiederherstellung des Diensts durch Angabe eines entsprechenden Werts für [QuorumLossWaitDuration](https://docs.microsoft.com/powershell/module/servicefabric/update-servicefabricservice?view=azureservicefabricps) unterstützt werden. Service Fabric wartet die durch QuorumLossWaitDuration angegebene Zeit (der Standardwert ist unendlich), bevor die Wiederherstellung durchgeführt wird. Diese Methode wird _nicht empfohlen_, da sie zu unerwarteten Datenverlusten führen kann.
