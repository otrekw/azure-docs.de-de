...
1. Wenn Sie so weit sind, klicken Sie auf **Registrieren**.
    ...
    ![Bildschirm zum Registrieren einer neuen Anwendung im Azure-Portal](../media/new-app-registration.png)

Azure AD weist Ihrer App eine eindeutige Anwendungs-ID (oder Client-ID) zu. Im Portal wird die Seite **Übersicht** für Ihre Anwendung geöffnet. Notieren Sie sich den Wert von **Anwendungs-ID (Client-ID)** .

Wenn Sie Ihrer Anwendung Funktionen hinzufügen möchten, können Sie weitere Konfigurationsoptionen wie Branding, Zertifikate und Geheimnisse, API-Berechtigungen und Ähnliches auswählen.

![Beispiel der Übersichtsseite einer neu registrierten App](../media//new-app-overview-page-expanded.png)

Bevor Sie Aufrufe von Ihrer Anwendung an den Schlüsseltresor durchführen können, müssen Sie den Schlüsseltresor über Ihre Anwendung und deren Berechtigungen informieren. Der folgende Befehl verwendet den Namen des Tresors und die **Anwendungs-ID (Client-ID)** aus Ihrer Azure Active Directory-App, um der Anwendung den Zugriff zum **Abrufen** auf Ihren Schlüsseltresor zu gewähren.
...
Zunächst müssen Sie die Protokollierung für Ihren Schlüsseltresor aktivieren. Verwenden Sie die folgenden PowerShell-Befehle. (Bitte entnehmen Sie die vollständigen Details [diesem Artikel zur Schlüsseltresorprotokollierung](../general/logging.md).)
