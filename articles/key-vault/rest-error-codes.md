...
Aktivieren Sie zunächst die Protokollierung. Anleitungen hierzu finden Sie unter [Azure Key Vault-Protokollierung](key-vault-logging.md).
...
Im Allgemeinen sind Anforderungen an den Key Vault auf 2.000 Anforderungen/10 Sekunden beschränkt. Ausnahmen sind wichtige Vorgänge, die unter [Limits des Key Vault-Diensts](key-vault-service-limits.md) dokumentiert werden.
...
- Wenn Key Vault-Drosselung auftritt, passen Sie den anfordernden so Code an, dass er ein exponentielles Backoff für Wiederholungsversuche verwendet. Der Algorithmus wird hier erläutert: [Drosselung Ihrer App](key-vault-ovw-throttling.md#how-to-throttle-your-app-in-response-to-service-limits)
...
Ausführliche Anleitungen einschließlich der Anforderung zur Erhöhung der Limits finden Sie hier: [Anleitung zur Drosselung von Key Vault](key-vault-ovw-throttling.md)
