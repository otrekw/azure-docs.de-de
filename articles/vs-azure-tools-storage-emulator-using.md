...
description: Konfigurieren und Verwenden des Speicheremulators, ein Dienstprogramm, das die in Azure verfügbaren Blob-, Warteschlangen- und Tabellenspeicherdienste auf dem lokalen Entwicklungscomputer simuliert.
...
Sie können angeben, dass beim Ausführen oder Debuggen des Diensts in Visual Studio automatisch der Speicheremulator gestartet werden soll. Öffnen Sie im Projektmappen-Explorer das Kontextmenü für das **Azure**-Projekt, und wählen Sie **Eigenschaften** aus. Wählen Sie auf der Registerkarte **Entwicklung** in der Liste **Azure-Speicheremulator starten** die Option **True** aus (falls sie noch nicht auf diesen Wert festgelegt ist).  Einige Projekttypen haben nicht die Registerkarte **Entwicklung**. Wenn dies der Fall ist, können Sie den Start des Speicheremulators aktivieren bzw. deaktivieren, indem Sie das `StartDevelopmentStorage`-Element in der Projektdatei festlegen. Setzen Sie es zum Aktivieren auf **True** bzw. auf **False** zum Deaktivieren.  Öffnen Sie z. B. in einem Azure Functions-Projekt die Projektdatei zur Bearbeitung und ändern Sie den XML-Code wie folgt:
