---
title: Datenaufbereitung mit Spark-Pools (Vorschau)
titleSuffix: Azure Machine Learning
description: Hier erfahren Sie, wie Sie Spark-Pools für die Datenaufbereitung mit Azure Synapse und Azure Machine Learning anfügen.
...
ms.locfileid: "101660362"
---
# <a name="attach-synapse-spark-pools-for-data-preparation-with-azure-synapse-preview"></a>Anfügen von Synapse Spark-Pools für die Datenaufbereitung mit Azure Synapse (Vorschau)

In diesem Artikel erfahren Sie, wie Sie einen von [Azure Synapse](/synapse-analytics/overview-what-is.md) unterstützten Apache Spark-Pool für die Datenaufbereitung anfügen und starten. 

>[!IMPORTANT]
> Die Integration von Azure Machine Learning und Azure Synapse befindet sich in der Vorschauphase. Die in diesem Artikel vorgestellten Funktionen verwenden das Paket `azureml-synapse`, das [experimentelle](/python/api/overview/azure/ml/?preserve-view=true&view=azure-ml-py#stable-vs-experimental) Previewfunktionen enthält, die sich jederzeit ändern können.

## <a name="azure-machine-learning-and-azure-synapse-integration-preview"></a>Integration von Azure Machine Learning und Azure Synapse (Vorschau)

Die Integration von Azure Synapse in Azure Machine Learning (Vorschau) ermöglicht Ihnen das Anfügen eines Apache Spark-Pools, der von Azure Synapse für die interaktive Datenuntersuchung und -aufbereitung unterstützt wird. Dank dieser Integration können Sie eine dedizierte Computeressource für die Datenaufbereitung im großen Stil innerhalb des gleichen Python-Notebooks nutzen, das Sie auch zum Trainieren Ihrer Machine Learning-Modelle verwenden.
...
## <a name="link-machine-learning-workspace-and-synapse-assets"></a>Verknüpfen des Machine Learning-Arbeitsbereichs mit Synapse-Ressourcen

Bevor Sie einen Synapse Spark-Pool für die Datenaufbereitung anfügen können, muss der Azure Machine Learning-Arbeitsbereich mit Ihrem Azure Synapse-Arbeitsbereich verknüpft werden. 
...
>
> Wenn Sie kein **Besitzer** des Synapse-Arbeitsbereichs sind, aber einen vorhandenen verknüpften Dienst verwenden möchten, lesen Sie die Informationen unter [Abrufen eines vorhandenen verknüpften Diensts](#get-an-existing-linked-service).
...
Zum Abrufen und Verwenden eines vorhandenen verknüpften Diensts sind Berechtigungen vom Typ **Benutzer oder Mitwirkender** für den Synapse-Arbeitsbereich erforderlich.
...
Wenn Ihre Arbeitsbereiche verknüpft sind, fügen Sie einen Synapse Spark-Pool als dedizierte Computeressource für Ihre Datenaufbereitungsaufgaben an. 

Sie können Synapse Spark-Pools über folgende Komponenten anfügen:
...
Führen Sie die folgenden Schritte aus, um einen Synapse Spark-Pool mithilfe von Studio anzufügen. 
...
1. Wählen Sie Ihren Synapse Spark-Pool in der Liste aus, und geben Sie einen Namen an.  
...
Sie können einen Synapse Spark-Pool auch mithilfe des **Python SDK** anfügen. 
...
1. Konfigurieren von SynapseCompute mit
   ...
   1. dem Namen des Synapse Spark-Pools. Dieser muss einem vorhandenen Apache Spark-Pool entsprechen, der sich in Ihrem Synapse-Arbeitsbereich befindet.
...
1. Erstellen eines Machine Learning-Elements vom Typ „ComputeTarget“ durch Übergabe 
   ...
   1. des Namens, mit dem Sie im Machine Learning-Arbeitsbereich auf die Computeressource verweisen möchten 
   1. der Anfügekonfiguration (attach_configuration), die Sie beim Konfigurieren von SynapseCompute angegeben haben
...
Überprüfen Sie, ob der Synapse Spark-Pool angefügt wurde.
...
Sie können eine [Azure Machine Learning-Umgebung](concept-environments.md) angeben, die während der Synapse-Sitzung verwendet werden soll. Nur in der Umgebung angegebene Conda-Abhängigkeiten werden wirksam. Ein Docker-Image wird nicht unterstützt.

>[!WARNING]
>  In Conda-Umgebungsabhängigkeiten angegebene Python-Abhängigkeiten werden in Synapse Spark-Pools nicht unterstützt. Derzeit werden nur feste Python-Versionen unterstützt. Überprüfen Sie Ihre Python-Version, indem Sie `sys.version_info` in Ihr Skript einschließen.

Der folgende Code erstellt die-Umgebung `myenv`, in der vor Beginn der Sitzung Version 1.20.0 von `azureml-core` und Version 1.17.0 von `numpy` installiert werden. Anschließend können Sie diese Umgebung in die `start`-Anweisung der Synapse-Sitzung einschließen.
...
Geben Sie zum Starten der Datenaufbereitung mit dem Synapse Spark-Pool den Namen des Synapse Spark-Pools sowie Ihre Abonnement-ID, die Ressourcengruppe des Machine Learning-Arbeitsbereichs, den Namen des Machine Learning-Arbeitsbereichs und die während der Synapse-Sitzung zu verwendende Umgebung an. 

> [!IMPORTANT]
> Wenn Sie den Synapse Spark-Pool weiterhin verwenden möchten, müssen Sie angeben, welche Computeressource in den Datenaufbereitungsaufgaben verwendet werden soll. Verwenden Sie `%synapse` für einzelne Codezeilen und `%%synapse` für mehrere Zeilen. 
...
Lesen Sie nach dem Starten der Synapse Spark-Sitzung die Daten ein, die Sie aufbereiten möchten. Das Laden von Daten wird für Azure Blob Storage and Azure Data Lake Storage Gen1 und Gen2 unterstützt.
...
Wenn die Datenaufbereitung abgeschlossen ist und die aufbereiteten Daten im Speicher gespeichert wurden, beenden Sie die Verwendung Ihres Synapse Spark-Pools mit dem folgenden Befehl:
...
In [diesem End-to-End-Notebook](../synapse-analytics/overview-what-is.md) finden Sie ein ausführliches Codebeispiel für die Durchführung der Datenaufbereitung und des Modelltrainings über ein einzelnes Notebook mit Azure Synapse und Azure Machine Learning.
