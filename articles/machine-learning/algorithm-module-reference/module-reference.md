...
| Datentransformation | Vorgänge für Daten, die für maschinelles Lernen typisch sind, z. B. Normalisieren oder Quantisierung von Daten, Verringerung der Dimensionalität und Konvertierung von Daten zwischen verschiedenen Dateiformaten.| [Hinzufügen von Spalten](add-columns.md) <br/> [Hinzufügen von Zeilen](add-rows.md) <br/> [Bereinigen fehlender Daten](clean-missing-data.md) <br/> [Konvertieren in CSV](convert-to-csv.md) <br/> [Konvertieren in ein Dataset](convert-to-dataset.md) <br/> [Bearbeiten von Metadaten](edit-metadata.md) <br/> [Verknüpfen von Daten](join-data.md) <br/> [Normalisieren von Daten](normalize-data.md) <br/> [Entfernen doppelter Zeilen](remove-duplicate-rows.md) <br/> [Auswählen der Spaltentransformation](select-columns-transform.md) <br/> [Auswählen von Spalten im Dataset](select-columns-in-dataset.md) |
| Stichproben | Teilen Sie die Daten in kleinere Teilmengen auf, um Machine Learning-Modelle zu trainieren und zu testen.  | [Cross Validate Model](cross-validate-model.md) <br/> [Partition und Beispiel](partition-and-sample.md) <br/> [SMOTE](smote.md) <br/> [Aufteilen von Daten](split-data.md) |
...
| Python und R | Schreiben Sie Code, und betten Sie ihn in ein Modul ein, um Python und R in Ihre Pipeline zu integrieren. | [Erstellen eines Python-Modells](create-python-model.md) <br/> [Ausführen von Python-Skripts](execute-python-script.md)   <br/>  [Ausführen von R-Skripts](execute-r-script.md)
...
| Trainieren   | Führen Sie Daten über den Algorithmus aus. | [Train Model](train-model.md) (Modell trainieren)  <br/> [Trainieren des Clusteringmodells](train-clustering-model.md) <br/>  [Tune Model Hyperparameters](tune-model-hyperparameters.md) |
...
| Statistische Funktionen | Stellen eine Vielzahl von statistischen Methoden für Data Science bereit. | [Anwenden einer mathematischen Operation](apply-math-operation.md) <br/> [Zusammenfassen von Daten](summarize-data.md)|
