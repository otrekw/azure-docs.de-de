...
Import/Export ist ein Vorgang der Azure Cache for Redis-Datenverwaltung. Mit diesem Vorgang können Sie Daten in Azure Cache for Redis importieren oder aus Azure Cache for Redis exportieren. Hierzu importieren bzw. exportieren Sie eine Momentaufnahme der Azure Cache for Redis-Datenbank (RDB) aus einem Premium-Cache in ein Blob in einem Azure Storage-Konto.
...
1. Um ein oder mehrere exportierte Cacheblobs zu importieren, [navigieren Sie im Azure-Portal zum Cache](cache-configure.md#configure-azure-cache-for-redis-settings), und wählen Sie im **Ressourcenmenü** die Option **Daten importieren** aus.
...
1. Um den aktuellen Inhalt des Cache in den Speicher zu exportieren, [navigieren Sie im Azure-Portal zum Cache](cache-configure.md#configure-azure-cache-for-redis-settings), und wählen Sie im **Ressourcenmenü** die Option **Daten exportieren** aus.
...
3. Wählen Sie den gewünschten Blobcontainer und dann **Auswählen** aus. Wählen Sie zum Verwenden eines neuen Containers die Option **Container hinzufügen** aus, damit Sie diesen zuerst hinzufügen und dann in der Liste auswählen können.
...
Ja. Sie können Daten, die aus Azure Cache for Redis-Instanzen exportiert wurden, sowie RDB-Dateien von Redis-Servern in beliebigen Clouds oder Umgebungen importieren. Zu den Umgebungen zählen Linux, Windows oder Cloudanbieter wie Amazon Web Services. Laden Sie zum Importieren dieser Daten die RDB-Datei von dem gewünschten Redis-Server in ein Seiten- oder Blockblob in einem Azure Storage-Konto. Importieren Sie sie dann in Ihre Premium-Instanz von Azure Cache for Redis. Sie möchten beispielsweise die Daten aus dem Produktionscache exportieren und in einen Cache importieren, der als Teil der Stagingumgebung zum Testen oder für die Migration verwendet wird.
