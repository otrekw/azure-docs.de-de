...
description: Die Sicherheitsbaseline für Azure Database for PostgreSQL-Einzelserver enthält Anleitungen und Ressourcen für die Vorgehensweise bei der Implementierung der Sicherheitsempfehlungen aus dem Azure-Sicherheitsvergleichstest.
author: msmbaldwin
...
Diese Sicherheitsbaseline wendet Empfehlungen aus dem [Azure-Sicherheitsvergleichstest Version 1.0](../security/benchmarks/overview-v1.md) auf Azure Database for PostgreSQL (Einzelserver) an. Der Azure-Sicherheitsvergleichstest enthält Empfehlungen zum Schutz Ihrer Cloudlösungen in Azure. Der Inhalt wird nach den **Sicherheitssteuerungen** gruppiert, die durch den Azure-Sicherheitsvergleichstest und die entsprechenden für Azure Database for PostgreSQL (Einzelserver) geltenden Empfehlungen definiert sind. 

> [!NOTE]
> Nicht auf Azure Database for PostgreSQL (Einzelserver) anwendbare **Steuerungen** oder Steuerungen, für die Microsoft die Verantwortung trägt, wurden ausgeschlossen. Die vollständige Zuordnung von Azure Database for PostgreSQL (Einzelserver) zum Azure-Sicherheitsvergleichstest finden Sie in der **[vollständigen Zuordnungsdatei der Azure Database for PostgreSQL (Einzelserver)-Sicherheitsbaseline](https://github.com/MicrosoftDocs/SecurityBenchmarks/raw/master/Azure%20Offer%20Security%20Baselines/1.1/azure-database-for-postgresql-single-server-security-baseline-v1.1.xlsx)** .
...
- [Erstellen und Verwalten von VNET-Dienstendpunkten und VNET-Regeln im Azure Database for PostgreSQL-Server](howto-manage-vnet-using-portal.md)

- [Konfigurieren von Azure Database for PostgreSQL-Firewallregeln](howto-manage-firewall-using-portal.md)
...
- [Konfigurieren von Serverprotokollen für Azure Database for PostgreSQL und Zugreifen auf diese](howto-configure-server-logs-in-portal.md)
...
- [Azure-Ressourcenprotokolle](https://docs.microsoft.com/azure/azure-monitor/essentials/resource-logs#send-to-azure-storage)
...
- [Aktivieren von Advanced Threat Protection für Azure Database for PostgreSQL](howto-database-threat-protection-portal.md)
...
**Leitfaden**: Azure Active Directory (Azure AD) und Azure Database for PostgreSQL (Einzelserver) verfügen nicht über das Konzept der Standardkennwörter.
...
- [Erstellen zusätzlicher Konten für Azure Database for PostgreSQL](howto-create-users.md)
...
**Leitfaden**: Die Anmeldung bei Azure Database for PostgreSQL (Einzelserver) wird sowohl über Angaben für Benutzername/Kennwort, die direkt in der Datenbank konfiguriert werden, als auch über eine Azure Active Directory-Identität (Azure AD) und Verwendung eines Azure AD-Tokens für die Herstellung einer Verbindung unterstützt. Bei Verwenden eines Azure AD-Tokens werden verschiedene Methoden unterstützt, wie z. B. ein Azure AD-Benutzer, eine Azure AD-Gruppe oder eine Azure AD-Anwendung, die eine Verbindung mit der Datenbank herstellt.

Getrennt davon ist der Zugriff auf die Steuerungsebene von PostgreSQL über die REST-API verfügbar, wobei einmaliges Anmelden unterstützt wird. Legen Sie den Autorisierungsheader für Ihre Anforderungen auf ein JSON-Webtoken fest, das Sie aus Azure AD abrufen.

- [Verwenden von Azure AD für die Authentifizierung bei Azure Database for PostgreSQL](howto-configure-sign-in-aad-authentication.md)

- [Grundlegendes zur REST-API für Azure Database for PostgreSQL](/rest/api/postgresql/)
...
**Leitfaden**: Aktivieren Sie die mehrstufige Authentifizierung für Azure Active Directory (Azure AD), und befolgen Sie die Empfehlungen für die Identitäts- und Zugriffsverwaltung in Azure Security Center. Wenn Sie Azure AD-Token für die Anmeldung bei Ihrer Datenbank verwenden, können Sie für Datenbankanmeldungen eine mehrstufige Authentifizierung anfordern.
...
- [Einrichten von Advanced Threat Protection für Azure Database for PostgreSQL](howto-database-threat-protection-portal.md)
...
**Leitfaden**: Überprüfen Sie die Azure Active Directory-Protokolle (Azure AD), um veraltete Konten zu ermitteln, die u. U. Azure Database for PostgreSQL-Administratorrollen enthalten können. Verwenden Sie zusätzlich Azure-Identitätszugriffsüberprüfungen, um Gruppenmitgliedschaften, den Zugriff auf Unternehmensanwendungen, die für den Zugriff auf Azure Database for PostgreSQL verwendet werden könnten, und Rollenzuweisungen effizient zu verwalten. Der Benutzerzugriff sollte regelmäßig überprüft werden (z. B. alle 90 Tage), um sicherzustellen, dass nur die richtigen Benutzer weiterhin Zugriff haben.
...
**Leitfaden**: Aktivieren Sie die Diagnoseeinstellungen für Azure Database for PostgreSQL und Azure Active Directory (Azure AD), und senden Sie alle Protokolle an einen Log Analytics-Arbeitsbereich. Konfigurieren Sie gewünschte Warnungen (z. B. fehlgeschlagene Authentifizierungsversuche) in Log Analytics.
...
Verwenden Sie Azure Active Directory (Azure AD) Identity Protection und Risikoerkennungsfeatures, um automatische Antworten auf erkannte verdächtige Aktionen zu konfigurieren. Sie können automatisierte Antworten über Azure Sentinel aktivieren, um die Sicherheitsmaßnahmen Ihrer Organisation zu implementieren.
...
- [Konfigurieren von Azure Database for PostgreSQL-Firewallregeln](concepts-firewall-rules.md)
...
- [Mindern der Datenexfiltration für Azure Database for PostgreSQL](concepts-data-access-and-security-private-link.md)
...
- [Konfigurieren der Verschlüsselung während der Übertragung für Azure Database for PostgreSQL](concepts-ssl-connection-security.md)
...
- [Erstellen von Benutzern in Azure Database for PostgreSQL: Hyperscale (Citus)](howto-create-users.md)
...
**Leitfaden**: Befolgen Sie die Empfehlungen im Azure Security Center zum Schützen Ihrer Azure Database for PostgreSQL-Instanzen und damit verbundenen Ressourcen.

Microsoft führt die Verwaltung der Sicherheitsrisiken auf den zugrunde liegenden Systemen aus, die Azure Database for PostgreSQL unterstützen.
...
- [Ablehnen eines bestimmten Ressourcentyps mit Azure Policy](/azure/governance/policy/samples/built-in-policies#general)
...
- [Sichern eines Servers in Azure Database for PostgreSQL](howto-restore-server-portal.md)

- [Grundlegendes zur Erstkonfiguration für Azure Database for PostgreSQL](tutorial-design-database-using-azure-portal.md)
...
- [Grundlegendes zu Sicherung und Wiederherstellung in Azure Database for PostgreSQL](concepts-backup.md)
