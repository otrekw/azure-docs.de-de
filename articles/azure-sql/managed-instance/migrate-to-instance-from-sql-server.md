---
title: Migration von SQL Server zu einer verwalteten Azure SQL-Instanz
description: Erfahren Sie, wie Sie eine Datenbank aus SQL Server zu Azure SQL Managed Instance migrieren.
...
- [Bewerten Sie die Azure SQL Managed Instance-Kompatibilität](#assess-sql-managed-instance-compatibility), wobei Sie sicherstellen sollten, dass es keine blockierenden Probleme gibt, die Ihre Migrationen verhindern können.
  - Dieser Schritt umfasst auch das Erstellen [einer Leistungsbaseline](#create-performance-baseline), um die Ressourcennutzung für Ihre SQL Server-Quellinstanz zu bestimmen. Er ist erforderlich, wenn Sie eine verwaltete SQL-Instanz von richtiger Größe bereitstellen und sicherstellen möchten, dass die Leistungen nach der Migration nicht beeinträchtigt sind.
- [Auswählen von App-Konnektivitätsoptionen](connect-application-instance.md)
- [Stellen Sie auf einer SQL Managed Instance mit optimaler Größe bereit](#deploy-to-an-optimally-sized-managed-instance), in der Sie technische Merkmale (Anzahl von virtuellen Kernen, verfügbarer Arbeitsspeicher) und Leistungsstufe („Unternehmenskritisch“, „Universell“) für Ihre SQL Managed Instance auswählen.
...
Wenn Sie alle erkannten Migrationsblocker aufgelöst haben und die Migration zu SQL Managed Instance fortsetzen, sollten Sie beachten, dass einige der Änderungen die Leistung Ihres Workloads beeinträchtigen können:
...
- Ein Tutorial zu einer Wiederherstellung aus einer Sicherung finden Sie unter [Erstellen einer Azure SQL Managed Instance](instance-create-quickstart.md).
- Ein Tutorial zur Migration mit DMS finden Sie unter [Migrieren Ihrer lokalen Datenbank zu Azure SQL Managed Instance mit DMS](../../dms/tutorial-sql-server-to-managed-instance.md).  
