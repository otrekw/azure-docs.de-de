...
Bei der Überwachung für [Azure SQL-Datenbank](sql-database-technical-overview.md) und [Azure Synapse Analytics](../synapse-analytics/sql-data-warehouse/sql-data-warehouse-overview-what-is.md) wird das Schreiben von Datenbankereignissen in ein [Azure Storage-Konto](../storage/common/storage-account-overview.md) hinter einem virtuellen Netzwerk und einer Firewall unterstützt. 

In diesem Artikel werden zwei Möglichkeiten erläutert, wie Sie Azure SQL Server und das Azure Storage-Konto für diese Option konfigurieren können. Beim ersten Verfahren wird das Azure-Portal und beim zweiten Verfahren REST verwendet.
...
> * Das Speicherkonto muss sich in demselben Abonnement und am gleichen Speicherort wie der Azure SQL-Datenbank-Server befinden. 
...
Stellen Sie mit Ihrem Abonnement eine Verbindung mit dem [Azure-Portal](https://portal.azure.com) her. Navigieren Sie zu der Ressourcengruppe und zum Azure SQL-Datenbankserver.
...
|`<sql database server>`| Name des Azure SQL-Datenbank-Servers|
|`<administrator login>`| SQL-Datenbankadministratorkonto |
...
1. Registrieren Sie Ihren Azure SQL-Datenbank-Server bei Azure Active Directory (Azure AD). Verwenden Sie entweder PowerShell oder die REST-API.
...
2. Öffnen Sie das [Azure-Portal](https://portal.azure.com). Navigieren Sie zum Speicherkonto. Suchen Sie **Zugriffssteuerung (IAM)** , und klicken Sie auf **Rollenzuweisung hinzufügen**. Weisen Sie dem Server, der die Azure SQL-Datenbank-Instanz hostet, die Sie im vorherigen Schritt bei Azure Active Directory (Azure AD) registriert haben, die RBAC-Rolle **Mitwirkender an Storage-Blobdaten** zu.
...
3. Konfigurieren Sie die [Blobüberwachungsrichtlinie von Azure SQL Server](/rest/api/sql/server%20auditing%20settings/createorupdate), ohne einen *storageAccountAccessKey* anzugeben:
...
- [Verwenden von PowerShell zum Erstellen eines VNET-Dienstendpunkts und einer VNET-Regel für Azure SQL-Datenbank](sql-database-vnet-service-endpoint-rule-powershell.md)
- [VNET-Regeln: Vorgänge mit REST-APIs](/rest/api/sql/virtualnetworkrules)
- [Verwenden von VNET-Dienstendpunkten und -Regeln für Datenbankserver](sql-database-vnet-service-endpoint-rule-overview.md)
